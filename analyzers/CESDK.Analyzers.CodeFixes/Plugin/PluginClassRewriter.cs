...
namespace CESDK.Analyzers.CodeFixes.Plugin;
...
internal static class PluginClassRewriter
{
    ...
    {
        var constructor = SyntaxFactory.ConstructorDeclaration(declaration.Identifier.WithoutTrivia())
    ...
    public static ConstructorDeclarationSyntax MakePublic(ConstructorDeclarationSyntax constructor)
    {
        ...
        var first = IndexOfAccessibility(modifiers, 0);
        if (first < 0)
        {
            ...
            var anchor = modifiers.Count > 0 ? modifiers[0] : constructor.Identifier;
            var publicKeyword = SyntaxFactory.Token(anchor.LeadingTrivia, SyntaxKind.PublicKeyword,
                SyntaxFactory.TriviaList(SyntaxFactory.Space));
            var stripped = constructor.ReplaceToken(anchor, anchor.WithLeadingTrivia(SyntaxFactory.TriviaList()));
            return stripped.WithModifiers(stripped.Modifiers.Insert(0, publicKeyword));
        }

        var original = modifiers[first];
        ...
        for (var next = IndexOfAccessibility(modifiers, first + 1);
             next >= 0;
             next = IndexOfAccessibility(modifiers, next))
        {
            var dropped = modifiers[next];
            var previous = modifiers[next - 1];
            var carried = FromFirstComment(dropped.LeadingTrivia.AddRange(dropped.TrailingTrivia));
            modifiers = modifiers
    ...
    {
        var kept = SyntaxFactory.TriviaList();
    ...
    {
        for (var index = start; index < modifiers.Count; index++)
