...
internal static class ExceptionGuardRewriter
{
	...
	public static BlockSyntax Guard(BlockSyntax body, ITypeSymbol returnType)
	{
		...
		BlockSyntax tryBlock = SyntaxFactory.Block(body.Statements)
	...
	{
		SyntaxTriviaList comments = CommentLines(expressionBody.ArrowToken.LeadingTrivia, endOfLine)
		...
		SyntaxTriviaList afterSemicolon = semicolon.TrailingTrivia;
		...
		SyntaxToken statementSemicolon = semicolon.WithTrailingTrivia(withStatement.Add(SyntaxFactory.ElasticMarker));
		ExpressionSyntax expression = expressionBody.Expression.WithoutLeadingTrivia();
		...
		BlockSyntax tryBlock = SyntaxFactory.Block(statement.WithLeadingTrivia(comments));
	...
	{
		SyntaxTriviaList comments = SyntaxFactory.TriviaList();
	...
	{
		SyntaxTriviaList slice = SyntaxFactory.TriviaList();
		for (int index = start; index < end; index++)
		{
			slice = slice.Add(trivia[index]);
		}
	...
	{
		TypeSyntax exceptionType = SyntaxFactory.ParseTypeName("global::System.Exception")
