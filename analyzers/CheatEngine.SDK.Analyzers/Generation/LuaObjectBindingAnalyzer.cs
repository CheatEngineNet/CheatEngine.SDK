...
public sealed class LuaObjectBindingAnalyzer : DiagnosticAnalyzer
{
	...
	private static void AnalyzeMethod(SymbolAnalysisContext context, LuaObjectContractSymbols symbols)
	{
			...
			&& FindAttribute(method, symbols.LuaMethodAttribute) is { } luaMethodAttribute)
		{
			...
			if (problem.Length > 0)
			{
				ReportInvalid(context, method, problem);
			}
		}
	}
	...
	private static void AnalyzeProperty(SymbolAnalysisContext context, LuaObjectContractSymbols symbols)
	{
		...
		string problem = LuaPropertyProblem(property, luaPropertyAttribute, symbols, context.CancellationToken);
		if (problem.Length > 0)
		{
			ReportInvalid(context, property, problem);
		}
	}
	...
	{
		if (LuaClassGeneratedNames.IsGeneratedType(type.Name))
		{
			ReportCollision(context, type, type.Name);
		}
	...
	{
		foreach (ISymbol member in type.GetMembers(sourceName))
		{
			ReportCollision(context, member, generatedIdentity);
		}
	}
	...
	{
		if (symbols.LuaFunctionAttribute is null)
		{
			return;
		}

		bool containsValidFunction = false;
		ContainingTypeIssues containingIssues = ContainingTypeShape.Inspect(type, context.CancellationToken);
		foreach (ISymbol member in type.GetMembers())
		{
			...
			if (issues != LuaFunctionShapeIssues.None || containingIssues != ContainingTypeIssues.None)
			{
				continue;
			}
	...
	private static void ReportLuaGlobalIdentityCollisions(SymbolAnalysisContext context, INamedTypeSymbol type,
		LuaObjectContractSymbols symbols)
	{
		if (symbols.LuaGlobalAttribute is null)
		{
			return;
		}
		...
		foreach (ISymbol member in type.GetMembers())
		{
			...
			if (issues != LuaGlobalShapeIssues.None || containingIssues != ContainingTypeIssues.None)
			{
				continue;
			}

			ReportLuaGlobalLocalCollisions(context, method);
			string generatedName = LuaGlobalCallModel.CacheFieldFor(name!);
		...
		CancellationToken cancellationToken)
	{
		...
		{
			return "the Lua class name must be a Lua identifier";
		}

		if (type.TypeKind != TypeKind.Struct)
		{
			return "[LuaClass] is supported only on a struct";
		}

		if (type.IsRecord)
		{
			return "the borrowed handle struct must not be a record struct";
		}

		if (type.IsRefLikeType)
		{
			return "the borrowed handle struct must not be ref-like";
		}

		if (!type.IsReadOnly)
		{
			return "the borrowed handle struct must be readonly";
		}

		if (type.Arity != 0)
		{
			return "the borrowed handle struct must not be generic";
		}

		if (type.IsFileLocal)
		{
			return "the borrowed handle struct must not be file-local";
		}

		if (!IsPartial(type, cancellationToken))
		{
			return "the borrowed handle struct must be partial";
		}
	...
	private static string ContainingLuaClassProblem(INamedTypeSymbol? containing, CancellationToken cancellationToken)
	{
		for (; containing is not null; containing = containing.ContainingType)
		{
			...
			{
				return "no containing type of the borrowed handle struct may be generic";
			}

			if (containing.IsFileLocal)
			{
				return "no containing type of the borrowed handle struct may be file-local";
			}
	...
	private static string LuaMethodProblem(IMethodSymbol method, AttributeData attribute,
		...
		CancellationToken cancellationToken)
	{
		string classProblem = LuaClassProblemForMember(method.ContainingType, symbols, cancellationToken);
		if (classProblem.Length > 0)
		{
			return classProblem;
		}
		...
		if (!LuaNames.IsValidName(ReadName(attribute)))
		{
			return "the Lua method name must be a Lua identifier";
		}

		if (method.MethodKind != MethodKind.Ordinary)
		{
			return "the member must be an ordinary method";
		}

		if (method.IsStatic)
		{
			return "the generated object method needs an instance receiver";
		}

		if (method.IsGenericMethod)
		{
			return "the generated object method must not be generic";
		}

		if (method.IsAsync)
		{
			return "the generated object method must not be async";
		}

		if (!method.IsPartialDefinition)
		{
			return "the member must be the defining declaration of a partial method";
		}

		if (method.PartialImplementationPart is not null)
		{
			return "the partial method already has an implementation";
		}
		...
		string parameterProblem = LuaMethodParameterProblem(method, symbols, cancellationToken, out bool hasOutResult);
		if (parameterProblem.Length > 0)
		{
	...
	private static string LuaMethodParameterProblem(IMethodSymbol method, LuaObjectContractSymbols symbols,
		CancellationToken cancellationToken, out bool hasOutResult)
	{
		hasOutResult = false;
		foreach (IParameterSymbol parameter in method.Parameters)
		{
			cancellationToken.ThrowIfCancellationRequested();
			if (IsGeneratedLocalName(parameter.Name))
			{
				return "the parameter name collides with a generated local";
			}
			...
			if (hasOutResult)
			{
				return "arguments must precede every out result";
			}

			if (parameter.RefKind != RefKind.None)
			{
				return "ref, in and ref readonly parameters are not supported";
			}
	...
	private static string LuaPropertyProblem(IPropertySymbol property, AttributeData attribute,
		...
		CancellationToken cancellationToken)
	{
		string classProblem = LuaClassProblemForMember(property.ContainingType, symbols, cancellationToken);
		if (classProblem.Length > 0)
		{
			return classProblem;
		}
		...
		if (!LuaNames.IsValidName(ReadName(attribute)))
		{
			return "the Lua property name must be a Lua identifier";
		}

		if (property.IsStatic)
		{
			return "the generated object property needs an instance receiver";
		}
		...
		INamedTypeSymbol? ceObject)
	{
		...
		public INamedTypeSymbol? CEObject
		{
			get;
		} = ceObject;
