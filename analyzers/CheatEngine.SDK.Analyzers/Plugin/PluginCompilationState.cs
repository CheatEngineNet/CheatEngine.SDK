...
internal sealed class PluginCompilationState
{
	...
	public void Report(CompilationAnalysisContext context)
	{
		...
		{
			ReportGeneratedEntryPointDiagnostics(context, pluginClassCount);
			return;
		}

		if (_entryPointIsGenerated is false)
		{
			ReportMissingManualBootstrap(context);
		}
	}

	private void ReportGeneratedEntryPointDiagnostics(CompilationAnalysisContext context, int pluginClassCount)
	{
		if (pluginClassCount > 1)
		{
			foreach ((string name, Location location) in _pluginClasses)
			{
		...
		foreach ((string name, Location location) in _reservedNamespaces)
		{
			context.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.ReservedNamespace, location, name));
		}

		foreach ((string name, Location location, bool _) in _entryPointTypes)
		{
	...
	private void ReportMissingManualBootstrap(CompilationAnalysisContext context)
	{
		foreach ((string _, Location _, bool isManualBootstrap) in _entryPointTypes)
		{
		...
		foreach ((string _, Location location, bool _) in _entryPointTypes)
		{
			locationForManualBootstrap = location;
			break;
		}

		string requirement = locationForManualBootstrap == Location.None
