	...
	INamedTypeSymbol? environmentType)
{
		...
		INamedTypeSymbol? doesNotReturnAttribute, INamedTypeSymbol? environmentType)
	{
		...
		foreach (ICatchClauseOperation catchClause in tryOperation.Catches)
		{
			...
			if (ContainsThrow(catchClause.Handler, doesNotReturnAttribute, environmentType))
			{
				return false;
			}
	...
	private static bool IsNonThrowingConversion(IConversionOperation operation)
	{
		...
		if (conversion.IsIdentity || conversion.IsDefaultLiteral || conversion.IsNullLiteral)
		{
			return true;
		}

		ITypeSymbol? source = operation.Operand.Type;
		ITypeSymbol? target = operation.Type;
