...
public sealed class PluginLifecycleAndOwnershipAnalyzer : DiagnosticAnalyzer
{
	...
	private static void OnCompilationStart(CompilationStartAnalysisContext context)
	{
		...
		INamedTypeSymbol? ceOwned =
			SdkSymbolResolver.Annotation(context.Compilation, WellKnownTypeNames.CEOwnedAttribute);
		if (pluginAttribute is null || pluginBase is null || (requiresPluginEnabled is null && ceOwned is null))
		{
			return;
		}
	...
	private static void AnalyzeInvocation(OperationAnalysisContext context, PluginLifecycleContractSymbols symbols)
	{
		...
		if (symbols.CEOwned is null || !IsDisposal(invocation.TargetMethod) || invocation.Instance is null)
		{
			return;
		}

		if (!IsExplicitlyBorrowed(invocation.Instance, symbols.CEOwned))
		{
			return;
		}
		...
		PluginLifecycleContractSymbols symbols)
	{
		...
		IPropertyReferenceOperation property = (IPropertyReferenceOperation) context.Operation;
		if (!RequiresEnabled(property.Property, symbols.RequiresPluginEnabled))
		{
			return;
		}
	...
	private static void AnalyzeMethod(SymbolAnalysisContext context, PluginLifecycleContractSymbols symbols)
	{
		...
		if (!IsLifecycleOverride(method, symbols.PluginBase))
		{
			return;
		}
	...
	{
		if (!IsPluginClass(containingSymbol.ContainingType, pluginAttribute))
		{
			return false;
		}
	...
	{
		if (method.Name is not "OnEnable" and not "OnDisable")
		{
			return false;
		}
	...
	{
		for (ISymbol? current = symbol; current is not null; current = OverriddenMember(current))
		{
	...
	private static bool HasOwnedReturn(IMethodSymbol method, INamedTypeSymbol ceOwned)
	{
		...
		{
			foreach (AttributeData attribute in current.GetReturnTypeAttributes())
			{
		...
		INamedTypeSymbol? ceOwned)
	{
		...
		public INamedTypeSymbol? RequiresPluginEnabled
		{
			get;
		} = requiresPluginEnabled;

		public INamedTypeSymbol? CEOwned
		{
			get;
		} = ceOwned;
	}
