...
A class carries `[CheatEngine.SDK.Annotations.Plugin.CheatEnginePlugin]`, but the entry point that the SDK generates
into the
...
- Projects in which `CheatEngine.SDK.Annotations.Plugin.CheatEnginePluginAttribute` or
  `CheatEngine.SDK.Hosting.Plugin.CheatEnginePlugin`
  cannot be resolved: the analyzer registers nothing there.
- Projects that switch the generated entry point off
  (`<CheatEngineSdkGenerateEntryPoint>false</CheatEngineSdkGenerateEntryPoint>`): the
