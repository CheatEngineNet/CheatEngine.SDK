...
| `MissingParameterlessConstructor`      | If constructors are declared, one of them is callable with an empty argument list: a parameterless constructor, one whose every parameter is optional, or one ending in a `params` parameter. These are the constructors `new T()` itself binds to. A primary constructor with an empty parameter list counts.                                                                 |
| `InaccessibleParameterlessConstructor` | At least one constructor is callable with an empty argument list, but every one of them is `private`, `protected` or `private protected`. A constructor without an accessibility keyword is `private`. When several constructors are callable with an empty argument list, only an accessible one matters: it is what `new T()` binds to.                                      |
...
One limit applies to `RequiredMembers` and `ObsoleteError`. When several constructors are callable with an empty
argument list, both are read from one of them only. That constructor is the first accessible one, or the first one when
none is accessible.
