...
| Timer and hotkey subscriptions | The SDK owns an internal, state-generation-bound lifecycle registry for a future qualified host subscription. It closes callback admission before `OnDisable`, unregisters in LIFO order while the old state is reachable, and never retries an unregister action. **It exposes no timer or hotkey API.** The classic timer declarations are historical inventory outside the managed export contract; the Lua hotkey path has no observable registration outcome and a source-level teardown-deadlock risk. | Deterministic SDK fixtures validate the owner only. No controlled CE 7.7 timer/hotkey profile has run, so affinity, registration outcome, enable/disable semantics and host-object destruction remain unavailable. |
| Target and host addresses | `Address` and `HostAddress` are distinct API types. Target-pointer calls accept an explicit `PointerSize`; an x86 value never derives its width from `IntPtr.Size`, and a value too wide for x86 is refused before the CE write. | Managed Lua-fixture tests, including controlled x86/x64 byte patterns; no live CE target was run. |
...
| Memory, inspection and scans | Scalar results are Lua numbers rather than coercible strings. Caller-owned byte and UTF-8 buffers have exact-capacity, short-buffer, partial-prefix, and CE-reported byte-count outcomes. | Pinned Lua fixture and managed tests; CE timing, affinity, and live transfer behavior are not inferred. |
