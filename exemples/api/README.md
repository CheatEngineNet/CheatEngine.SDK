...
| `CheatEngine.SDK.Engine.Assembly`                              | `AutoAssemblerPatcher`, `AutoAssemblerPatch`, and bounded profile-qualified instruction operations            | Auto Assembler remains a low-level owner; instruction operations are fixture-qualified and not a live-target claim |
| `CheatEngine.SDK.Engine.Scanning`, `.AddressList`              | AOB/StringList, `MemoryScanSessions` factory, scan sessions/results, borrowed address-list records           | [05](../05-aob-scans/README.md) to [07](../07-address-list/README.md)                                               |
...
direct-borrowed-dispose analysis. Ownership is never inferred from `class` versus `struct`: a Lua-class wrapper is
always borrowed, while an explicitly transferred object is `Owned<T>`. A malformed or colliding generator target is a
localized diagnostic, not silently ignored.
...
size, version, or record-capacity contract. The signature remains `CEPluginInitialize(IntPtr, int)` solely for ABI
compatibility pending exact CE 7.7 live evidence.

```mermaid
stateDiagram-v2
    [*] --> Uninitialized
    Uninitialized --> Registered: bootstrap succeeds
    Registered --> Enabling: enable starts
    Enabling --> Enabled: OnEnable succeeds
    Enabling --> Registered: enable cleanup/failure
    Enabled --> Disabling: disable starts
    Disabling --> Registered: cleanup completes
    Registered --> Enabling: later enable
```

`IsEnabled` is true only in stable `Enabled`. The host closes admission for new dispatched work before `Disabling`,
signals the context shutdown token, neutralizes callbacks, drains work admitted before that boundary, and detaches in
`finally`. A nested or concurrent lifecycle transition fails immediately rather than waiting under a re-entrant lock.
...
Any parameter or return value may override the built-in type mapping with a custom marshaller. Apply
`[LuaMarshaller(typeof(TMarshaller))]` to a parameter or use `[return: LuaMarshaller(typeof(TMarshaller))]` on the
method. The generator verifies that `TMarshaller` implements `ILuaMarshaller<T>` for the value's type and emits direct
calls to its static `Push` and `TryRead` members. This is type-checked, AOT-safe, and does not use reflection or a
runtime registry. You use it to pass custom value types, tokens or identifiers through Lua as integers or strings
without boxing.
...
or select a custom marshaller for a generated `[LuaFunction]` or `[LuaGlobal]` parameter with
`[LuaMarshaller(typeof(TMarshaller))]`. Persistent Lua resources compare both identity components. A supported SDK state
replacement invalidates old callbacks, references, and caches before rebinding; a direct external `resetLuaState` is
unsupported because the SDK cannot safely determine the fate of the old CE registry, userdata, or closures.
...
Ownership of an object whose factory or binding explicitly transfers ownership to your plugin. Obtain an owner only
from such an Engine API (for example, the AOB and `StringLists` factories); consumer code must never manufacture one
from a handle. `Dispose()` performs the documented `destroy()` path and there is no finalizer, so dispose an owner
before `OnDisable` returns. The contained `T` remains a borrowed handle; never put a Cheat-Engine-owned result such as
`getAddressList()` or `getCurrentMemscan()` in an owner. Transfer/release details are deliberately documented by each
factory rather than inferred from a handle type.
...
| Target and host memory   | `TargetMemory` takes target `Address`; `HostMemory` takes the distinct host address type, with scalar, pointer, byte-span, and text operations | Caller owns span storage; use observed `PointerSize` for target-qualified pointers, and consume detailed partial/count outcomes rather than guessing completion                                    |
...
| Auto Assembler           | `AutoAssemblerPatcher.TryApply` owns the returned `AutoAssemblerPatch` disable table; `AutoAssemblerPatch.Release` routes `[DISABLE]` once     | The patch consumes ownership before disable, so partial failure is never retried; Client availability remains live-gated                        |
| Instructions             | `InstructionProfiles`, `InstructionAssembler`, `InstructionDisassembler`, and `InstructionNavigator` use an explicit PID/ISA profile and structured outcomes | Returned bytes and disassembly fields are bounded caller/managed data; PID checks are coherence observations, not a CE target lock or live qualification |
...
| MemScan/FoundList        | `MemoryScanSessions.TryCreate` owns the scanner/child pair and rolls the parent back if child creation fails                                   | Dispose in child-before-parent order; it never hides asynchronous CE state changes; Client availability remains live-gated                      |
...
`CheatEngine.SDK.Hosting.Threading.MainThread` is the thread captured by a successful plugin enable. Treat it as the
Cheat Engine GUI boundary, but keep the distinction clear: the SDK thunk verifies the captured managed thread, while the
