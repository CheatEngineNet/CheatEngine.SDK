...
/// <remarks>
///     <para>
///         The sizes and offsets documented on the structures of this assembly (and asserted by <c>CESDK.Abi.Tests</c>)
///         are
///         the 64-bit ones. The structures themselves are declared with pointer-sized fields, so they would also describe
///         a 32-bit Cheat Engine, but that configuration is neither tested nor supported.
///     </para>
///     <para>
    ...
    internal static void ThrowIfUnsupported(Architecture architecture)
    {
        if (!IsSupportedArchitecture(architecture))
            throw new PlatformNotSupportedException(
                $"CESDK.Abi is validated for x64 processes only; this process is {architecture}.");
    }
}
