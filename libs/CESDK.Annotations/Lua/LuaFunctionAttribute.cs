...
/// <remarks>
///     <para>
///         <b>Consumed by.</b> The <c>CESDK.SourceGenerators.LuaBindings</c> generator, which discovers the method with
///         <c>ForAttributeWithMetadataName("CESDK.Annotations.Lua.LuaFunctionAttribute")</c>. Its contract for an
///         annotated
///         method is an <c>[UnmanagedCallersOnly]</c> <c>cdecl</c> thunk of the <c>lua_CFunction</c> shape that reads the
///         arguments from the Lua stack, calls the method inside a catch-all, pushes the result and reports failures
///         through
///         the SDK's managed-to-Lua error channel, plus an entry in a generated registration table that binds the thunk to
///         <see cref="Name" />. Which method shapes and parameter types it accepts is documented with the generator. A
///         method
///         it cannot export (not static, generic, unsupported parameter types, duplicate name) is a generator-input error
///         (CESDK2003); CESDK2001 is the error for a compilation that does not allow the unsafe code the thunks need.
///     </para>
///     <para>
