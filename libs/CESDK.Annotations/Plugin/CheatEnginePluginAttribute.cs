...
/// <remarks>
///     <para>
///         <b>Consumed by.</b> The <c>CESDK.SourceGenerators.EntryPoint</c> generator finds the class with
///         <c>ForAttributeWithMetadataName("CESDK.Annotations.Plugin.CheatEnginePluginAttribute")</c> and emits, into the
///         plugin
///         assembly, the <c>CESDK.CESDK.CEPluginInitialize</c> bootstrap and a factory that constructs the class with
///         <see langword="new" /> through its parameterless constructor (no reflection) and exposes <see cref="Name" /> as
///         a
///         UTF-8 literal. <c>CESDK.Analyzers</c> uses the same attribute to check that the class can be constructed that
///         way
