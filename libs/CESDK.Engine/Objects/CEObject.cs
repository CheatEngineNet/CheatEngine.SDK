...
public readonly struct CEObject : IEquatable<CEObject>, ICEObject<CEObject>, ILuaMarshaller<CEObject>
{
    ...
    public static unsafe bool TryRead(LuaState state, int index, out CEObject value)
    {
        ...
        {
            var block = state.ToUserdata(index);
            if (block != 0 && state.RawLength(index) >= (nuint)sizeof(nint))
            {
                var pointer = *(nint*)block;
                if (pointer != 0)
                {
    ...
    public LuaStatus TryGetProperty(LuaState state, ReadOnlySpan<byte> name)
    {
        ...
        Push(state);
        var status = state.TryGetField(-1, name);
        state.Remove(-2);
        return status;
    }
    ...
    public LuaStatus TrySetProperty(LuaState state, ReadOnlySpan<byte> name)
    {
        ...
        Push(state);
        state.Insert(-2);
        var status = state.TrySetField(-2, name);
    ...
    {
        Push(state);
        var status = state.TryGetIndex(-1, zeroBasedIndex);
        state.Remove(-2);
        return status;
    }
    ...
    public LuaStatus TrySetIndex(LuaState state, int zeroBasedIndex)
    {
        Push(state);
        state.Insert(-2);
        var status = state.TrySetIndex(-2, zeroBasedIndex);
        RemoveObjectAfterSet(state, status);
        return status;
    }
    ...
    {
        var status = TryGetProperty(state, name);
        if (status.IsOk && !state.IsFunction(-1)) return ReplaceWithNotAFunctionError(state, name);
    ...
    public LuaStatus TryCallMethod(LuaState state, ReadOnlySpan<byte> name, int argumentCount, int resultCount)
    {
        ...
        var status = TryPushMethod(state, name);
        if (argumentCount <= 0) return status.IsOk ? state.TryCall(argumentCount, resultCount) : status;
        state.Insert(-argumentCount - 1);
        if (!status.IsOk) state.Pop(argumentCount);
    ...
    {
        var status = TryGetPropertyLeavingObject(state, name);
        if (status.IsOk && !state.IsFunction(-1)) return ReplaceWithNotAFunctionError(state, name);
