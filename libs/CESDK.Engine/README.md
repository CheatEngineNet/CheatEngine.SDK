...
The typed Cheat Engine object model for CESDK plugins: object handles, explicit ownership, addresses, zero-based indices
and Cheat Engine's numeric constants.
...
|--------------------------|-----------------------------------------------|-------------------------------------------------------------------------------------------------------|
| `CESDK.Engine.Objects`   | `CEObject`, `ICEObject<TSelf>`                | Borrowed handle: the native object pointer, equal by identity, with property, index and method access |
| `CESDK.Engine.Objects`   | `Owned<T>`                                    | Ownership of an object the plugin created; `Dispose` destroys it                                      |
| `CESDK.Engine.Values`    | `Address`                                     | An address read from a Lua integer or hexadecimal text; its own Lua marshaller                        |
| `CESDK.Engine.Values`    | `IndexBase`, `LuaSequence`                    | Zero-based indices over Cheat Engine objects and Lua sequences                                        |
| `CESDK.Engine.Enums`     | Enums, `CEEnumNames`, `EnumMarshaller<TEnum>` | Numeric constants, their Cheat Engine names, and Lua integer marshalling                              |
| `CESDK.Engine.Generated` | `MemoryScalars`                               | Generated wrappers for `readInteger`, `writeInteger`, `readQword` and `writeQword`                    |
...
The repository-internal `CESDK.SourceGenerators.EngineApi` generates `MemoryScalars` at compile time from
`source-generators/CESDK.SourceGenerators.EngineApi/Specs/memory-scalars.cesdk-api.txt`. All four methods take an
`Address`. `TryReadInt32` and `TryReadInt64` return `false` when the read fails. `WriteInt32` and `WriteInt64` return
the flag Cheat Engine reports and throw `LuaException` when the Lua call fails.
...
The library references [`CESDK.Lua`](../CESDK.Lua/README.md), `CESDK.Lua.Interop` and `CESDK.Annotations`, and not [
`CESDK.Hosting`](../CESDK.Hosting/README.md). It reaches Cheat Engine through `LuaRuntime`, which `CESDK.Hosting`
attaches when the plugin is enabled. The assembly ships in the `CESDK` package under `lib/net10.0`.

## Promise

The tests in `tests/CESDK.Engine.Tests` drive a simulated Cheat Engine object model on a real Lua 5.3 state.
...
8. Enum values equal the numbers in Cheat Engine 7.7's `defines.lua`, and every name parses back (`EnumValueTests`,
   `CEEnumNamesTests`).
9. `CESDK.Engine.dll` and its XML documentation ship in the package, and the EngineApi generator never does
