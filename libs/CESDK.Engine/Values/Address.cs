    ...
    ILuaMarshaller<Address>
{
    ...
    {
        var digits = TrimAsciiWhitespace(utf8);
        if (digits.Length >= 2 && digits[0] == (byte)'0' && (digits[1] | 0x20) == (byte)'x') digits = digits[2..];
    ...
    {
        var digits = TrimAsciiWhitespace(text);
        if (digits.Length >= 2 && digits[0] == '0' && (digits[1] | 0x20) == 'x') digits = digits[2..];
        ...
        {
            var digit = c <= 0x7F ? HexDigitValue((byte)c) : -1;
            if (digit < 0 || value >> 60 != 0)
            {
                address = default;
                return false;
    ...
    {
        if (!TryParse(text, out var address)) ThrowFormat(text);
    ...
    {
        if (state.TryReadUtf8(index, out var utf8)) return TryParse(utf8, out value);

        if (state.TryReadInteger(index, out var bits))
        {
    ...
    {
        if (format.IsEmpty || (format[0] | 0x20) != 'x') ThrowUnsupportedFormat(format);
    ...
    private static ReadOnlySpan<byte> TrimAsciiWhitespace(ReadOnlySpan<byte> utf8)
    {
        var start = 0;
        var end = utf8.Length;
        while (start < end && IsAsciiWhitespace(utf8[start])) start++;

        while (end > start && IsAsciiWhitespace(utf8[end - 1])) end--;
    ...
    private static ReadOnlySpan<char> TrimAsciiWhitespace(ReadOnlySpan<char> text)
    {
        var start = 0;
        var end = text.Length;
        while (start < end && text[start] <= 0x7F && IsAsciiWhitespace((byte)text[start])) start++;

        while (end > start && text[end - 1] <= 0x7F && IsAsciiWhitespace((byte)text[end - 1])) end--;
    ...
    {
        if (b is >= (byte)'0' and <= (byte)'9') return b - '0';

        var lower = b | 0x20;
        if (lower is >= 'a' and <= 'f') return lower - 'a' + 10;
