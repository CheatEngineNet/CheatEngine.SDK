...
internal static unsafe partial class LuaProtectedApi
{
    ...
    private static int Invoke(lua_State* state, int operation, int inputCount, void* data, nuint size, nint first, nint second)
    {
        ...
        var status = cesdk_lua_protected(state, in exports, operation, inputCount, data, size, first, second);
    ...
    [LibraryImport("cesdk-lua-bridge", EntryPoint = "cesdk_lua_protected")]
    private static partial int cesdk_lua_protected(lua_State* state, in LuaProtectedExports exports, int operation, int inputCount, void* data, nuint size, nint first, nint second);

    [LibraryImport("cesdk-lua-bridge", EntryPoint = "cesdk_lua_bridge_abi_version")]
    private static partial uint cesdk_lua_bridge_abi_version();
    ...
    private static void ValidateAbiVersion()
    {
        ...
        try
        {
            actual = cesdk_lua_bridge_abi_version();
        }
        catch (EntryPointNotFoundException exception)
        {
            throw new InvalidOperationException(
                "cesdk-lua-bridge.dll is outdated: the ABI version export is missing. Restore or rebuild the matching CESDK package.",
                exception);
        }

        if (actual != ExpectedAbiVersion)
            throw new InvalidOperationException(
                $"cesdk-lua-bridge.dll has ABI version {actual}; this CESDK build requires version {ExpectedAbiVersion}.");
    }
}
