...
///     this assembly so that <c>CESDK.Engine</c> and generated code can reach the host's Lua state without referencing
///     <c>CESDK.Hosting</c> or the ABI.
/// </summary>
/// <remarks>
///     <para>
///         <b>Lifecycle.</b> <c>CESDK.Hosting</c> calls <see cref="Attach" /> in the enable callback, after binding
///         <c>CESDK.Lua.Interop.Api.LuaApi</c>, and <see cref="Detach" /> in the disable callback. Every attach advances
///         <see cref="Epoch" />, which invalidates every <see cref="References.LuaRef" /> created before it (disable and
///         re-enable,
///         and
...
public static unsafe class LuaRuntime
{
    ...
    /// <remarks>
    ///     Requires <c>CESDK.Lua.Interop.Api.LuaApi</c> to be bound already (asserted in Debug builds): every operation after
    ///     this call goes through it.
    /// </remarks>
    public static void Attach(in LuaHostBinding binding)
    {
        ...
        Debug.Assert(LuaApi.IsInitialized,
            "CESDK.Lua.Interop.Api.LuaApi must be bound before the runtime is attached.");
