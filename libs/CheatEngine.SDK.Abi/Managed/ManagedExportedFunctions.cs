...
public unsafe struct ManagedExportedFunctions
{
    ...
    ///     <c>CheatEngine.SDK.Lua.Interop</c>. The host creates one Lua thread per OS thread on demand (<i>inferred</i>
    ///     from the 7.5 host source; consistent with the official Lua binding, which re-queries the state on every access), so
