...
internal sealed unsafe class ClassicDebugEventDispatcher : IDisposable
{
    ...
    [ThreadStatic]
    private static ClassicDebugEventDispatcher? t_dispatcher;
        ...
        out ClassicDebugEventDispatcher? dispatcher)
    {
        ...
        if (!AbiArchitecture.IsSupported) return ClassicDebugEventRegistrationStatus.UnsupportedArchitecture;
        ...
        {
            if (s_active is not null) return ClassicDebugEventRegistrationStatus.AnotherRegistrationIsActive;
        ...
        try
        {
            ...
            *created._initialization = new DebugEventPluginInit { Callback = &DispatchUnmanaged };
            var functionId = created._registerFunction(pluginId, PluginType.OnDebugEvent, created._initialization);
            if (functionId < 0)
            {
    ...
    {
        if (ReferenceEquals(t_dispatcher, this)) return ClassicDebugEventReleaseStatus.CallbackIsExecuting;

        lock (_gate)
        {
            if (_released) return ClassicDebugEventReleaseStatus.Released;
            if (_releaseInProgress) return ClassicDebugEventReleaseStatus.ReleaseInProgress;
        ...
        if (initialization is not null) NativeMemory.Free(initialization);
    ...
    private static int Dispatch(void* nativeEvent)
    {
        ...
        if (nativeEvent is null) return 0;
        ...
        if (dispatcher is null || !dispatcher.TryEnterCallback(out var handler, out var observations)) return 0;

        var previous = t_dispatcher;
        t_dispatcher = dispatcher;
        try
        {
            var header = Unsafe.ReadUnaligned<DebugEventHeader>(nativeEvent);
