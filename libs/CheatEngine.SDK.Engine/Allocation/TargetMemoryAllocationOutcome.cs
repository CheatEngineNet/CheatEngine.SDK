...
public readonly record struct TargetMemoryAllocationOutcome
{
	...
	public TargetMemoryOperationOutcome Operation
	{
		get;
	}
	...
	public static TargetMemoryAllocationOutcome Succeeded(Address address)
	{
		if (address.IsZero)
		{
			throw new ArgumentException("A successful allocation outcome requires a nonzero target address.",
				nameof(address));
		}
	...
	public static TargetMemoryAllocationOutcome Failed(TargetMemoryOperationOutcome operation)
	{
		...
		{
			throw new ArgumentException("An unsuccessful allocation outcome requires a specified failure.",
				nameof(operation));
		}
