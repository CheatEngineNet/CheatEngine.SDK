...
public sealed class TargetMemoryAllocator
{
	...
	///     Cheat Engine accepted an allocation but an owner could not be published;
	///     <see cref="EngineResourceHandoffException.CleanupOutcome" />
	...
	internal AllocatedRegion AllocateCore(TargetAllocationRequest request, AllocatedRegionFactory factory)
	{
		...
		catch (Exception exception)
		{
			TargetReleaseOutcome cleanupOutcome =
				CompensateFailedPublication(targetBound, address, request.Size, incarnation);
	...
	public TargetMemoryAllocationOutcome AllocateWithOutcome(TargetAllocationRequest request)
	{
		...
		try
		{
			TargetMemoryAllocationOutcome outcome =
				targetBound.AllocateBoundWithOutcome(request, out _, out TargetSelectionObservation observation);
			if (!observation.IsQualified)
			{
				return TargetMemoryAllocationOutcome.Failed(TargetMemoryOperationOutcome.Failed(
					EngineFailureKind.TargetIdentityUnavailable));
	...
	{
		EngineMarshallingException cause = new("TargetMemoryAllocate", EngineMarshallingDirection.Result,
	...
	private static void ThrowUnqualifiedSuccessfulAllocation(TargetSelectionObservation observation)
	{
		...
		EngineTargetIdentityException cause = new("TargetMemoryAllocate", check);
