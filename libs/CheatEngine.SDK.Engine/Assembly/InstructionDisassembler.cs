...
public static class InstructionDisassembler
{
    ...
    /// <param name="instruction">The copied parsed result only when the returned status is <see cref="InstructionOperationStatus.Success" />.</param>
        ...
        int maximumUtf8Bytes, out InstructionDisassembly instruction, out int requiredUtf8Bytes)
    {
        ...
        try
        {
            ...
            var status = PushGlobal(state, SDisassemble, "disassemble"u8);
            ...
            if (!state.TryReadUtf8(-1, out var line)) return InstructionOperationStatus.InvalidResult;

            requiredUtf8Bytes = line.Length;
            if (line.Length > maximumUtf8Bytes) return InstructionOperationStatus.OutputTooLong;
            ...
            if (!state.TryCall(1, 4).IsOk) return InstructionOperationStatus.LuaFailure;

            if (!state.TryReadUtf8(-4, out var addressUtf8) ||
                !state.TryReadUtf8(-3, out var bytesUtf8) ||
                !state.TryReadUtf8(-2, out var opcodeUtf8) ||
                !state.TryReadUtf8(-1, out var extraUtf8))
                return InstructionOperationStatus.InvalidResult;

            var copiedLength = checked(addressUtf8.Length + bytesUtf8.Length + opcodeUtf8.Length + extraUtf8.Length);
            if (copiedLength > maximumUtf8Bytes) return InstructionOperationStatus.OutputTooLong;
            ...
            instruction = new InstructionDisassembly(address, Encoding.UTF8.GetString(addressUtf8),
                Encoding.UTF8.GetString(bytesUtf8), Encoding.UTF8.GetString(opcodeUtf8), Encoding.UTF8.GetString(extraUtf8),
                copiedLength);
