...
public static class InstructionNavigator
{
	...
	/// <param name="length">
	///     The positive byte length only when the status is <see cref="InstructionOperationStatus.Success" />
	///     .
	/// </param>
	...
	public static InstructionOperationStatus TryGetLength(InstructionTargetProfile targetProfile, Address address,
		out int length)
	{
		length = 0;
		...
		InstructionOperationStatus status = TryCallAddressGlobal(targetProfile, SGetInstructionSize,
			"getInstructionSize"u8, address, out long value);
	...
	///     The CE-observed selected PID and instruction profile used to validate input and output
	///     addresses.
	/// </param>
	/// <param name="address">The target address supplied to CE's <c>getPreviousOpcode</c> global.</param>
	/// <param name="previous">
	///     The estimated target address only when the status is
	///     <see cref="InstructionOperationStatus.Success" />.
	/// </param>
	/// <returns>A profile, availability, protected-call, or result-shape outcome.</returns>
	/// <exception cref="InvalidOperationException">The plugin is not enabled or the calling thread has no Lua state.</exception>
	[RequiresPluginEnabled]
	public static InstructionOperationStatus TryGetPrevious(InstructionTargetProfile targetProfile, Address address,
		out Address previous)
	{
		...
		InstructionOperationStatus status = TryCallAddressGlobal(targetProfile, SGetPreviousOpcode,
			"getPreviousOpcode"u8, address, out long value);
		if (status != InstructionOperationStatus.Success)
		{
			return status;
		}

		Address candidate = Address.FromInt64(value);
