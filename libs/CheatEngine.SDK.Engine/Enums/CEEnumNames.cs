...
public static class CEEnumNames
{
    ...
    public static bool TryParseCEName(ReadOnlySpan<byte> ceName, out VariableType value)
    {
        ...
        for (var candidate = VariableType.Byte; candidate <= VariableType.Grouped; candidate++)
    ...
    {
        for (var candidate = ScanOption.UnknownValue; candidate <= ScanOption.Unchanged; candidate++)
            if (ceName.SequenceEqual(candidate.ToCEName()))
            {
                value = candidate;
                return true;
    ...
    {
        for (var candidate = RoundingType.Rounded; candidate <= RoundingType.Truncated; candidate++)
            if (ceName.SequenceEqual(candidate.ToCEName()))
            {
                value = candidate;
                return true;
    ...
    {
        for (var candidate = FastScanMethod.NotAligned; candidate <= FastScanMethod.LastDigits; candidate++)
            if (ceName.SequenceEqual(candidate.ToCEName()))
            {
                value = candidate;
                return true;
    ...
    {
        for (var candidate = BreakpointMethod.Int3; candidate <= BreakpointMethod.Exception; candidate++)
            if (ceName.SequenceEqual(candidate.ToCEName()))
            {
                value = candidate;
                return true;
    ...
    {
        for (var candidate = BreakpointTrigger.Execute; candidate <= BreakpointTrigger.Write; candidate++)
            if (ceName.SequenceEqual(candidate.ToCEName()))
            {
                value = candidate;
                return true;
    ...
    {
        for (var candidate = ContinueMethod.Run; candidate <= ContinueMethod.StepOver; candidate++)
            if (ceName.SequenceEqual(candidate.ToCEName()))
            {
                value = candidate;
                return true;
    ...
    {
        for (var candidate = MemoryProtection.ReadOnly;
    ...
    {
        for (var candidate = DuplicateHandling.Ignore; candidate <= DuplicateHandling.Error; candidate++)
            if (ceName.SequenceEqual(candidate.ToCEName()))
            {
                value = candidate;
                return true;
