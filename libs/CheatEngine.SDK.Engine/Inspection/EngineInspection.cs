...
public static class EngineInspection
{
		...
		out int written)
	{
		...
		try
		{
			InspectionStatus status = PushGlobal(state, SEnumSectionsOfModule, "enumSectionsOfModule"u8);
		...
		out Address address)
	{
		...
		try
		{
			InspectionStatus status = PushGlobal(state, SGetAddressSafe, "getAddressSafe"u8);
			if (status != InspectionStatus.Success)
			{
				return status;
			}
		...
		out HostAddress address)
	{
		...
		try
		{
			...
			if (!Address.TryRead(state, -1, out Address resolved))
			{
	...
	public static InspectionStatus GetSymbolInfo(SymbolExpression expression, out SymbolInfo symbol)
	{
		...
		try
		{
			InspectionStatus status = PushGlobal(state, SGetSymbolInfo, "getSymbolInfo"u8);
			if (status != InspectionStatus.Success)
			{
				return status;
			}
	...
	public static InspectionStatus EnumerateMemoryRegions(Span<MemoryRegionInfo> destination, out int written)
	{
		...
		try
		{
			InspectionStatus status = PushGlobal(state, SEnumMemoryRegions, "enumMemoryRegions"u8);
			if (status != InspectionStatus.Success)
			{
				return status;
			}
	...
	public static InspectionStatus GetMemoryRegionInfo(Address address, out MemoryRegionInfo region)
	{
		...
		try
		{
			InspectionStatus status = PushGlobal(state, SGetMemoryRegionInfo, "getMemoryRegionInfo"u8);
			if (status != InspectionStatus.Success)
			{
				return status;
			}
		...
		TargetProcessId processId, bool hasProcessId)
	{
		...
		try
		{
			InspectionStatus status = PushGlobal(state, SEnumModules, "enumModules"u8);
			if (status != InspectionStatus.Success)
			{
				return status;
			}
			...
			if (!state.TryCall(hasProcessId ? 1 : 0, 1).IsOk)
			{
				return InspectionStatus.LuaFailure;
			}
		...
		out int written)
	{
		...
		InspectionStatus count = TryGetSequenceCount(state, tableIndex, out int sequenceCount);
		if (count != InspectionStatus.Success)
		{
			return count;
		}

		if (sequenceCount > destination.Length)
		{
			return InspectionStatus.DestinationTooSmall;
		}
	...
	{
		value = null;
		if (!TryGetField(state, tableIndex, field))
		{
			return false;
		}

		bool read = state.TryReadString(-1, out value);
		state.Pop(1);
		return read;
	}
	...
	private static bool TryReadAddressField(LuaState state, int tableIndex, ReadOnlySpan<byte> field, out Address value)
	{
		...
		bool read = Address.TryRead(state, -1, out value);
		state.Pop(1);
		return read;
	}
	...
	private static bool TryReadBooleanField(LuaState state, int tableIndex, ReadOnlySpan<byte> field, out bool value)
	{
		...
		bool read = state.TypeOf(-1) == LuaType.Boolean;
		if (read)
		{
	...
	{
		value = default;
		if (!TryReadUInt64Field(state, tableIndex, field, out ulong bytes))
		{
			return false;
		}
		...
		out MemorySize? value)
	{
		...
		bool read = state.TryReadInteger(-1, out long signed);
		state.Pop(1);
		if (!read || signed < 0)
		{
			return false;
	...
	{
		value = default;
		if (!TryReadUInt32Field(state, tableIndex, field, out uint raw))
		{
			return false;
		}
	...
	{
		value = 0;
		if (!TryReadUInt64Field(state, tableIndex, field, out ulong raw) || raw > uint.MaxValue)
		{
			return false;
		}
	...
	{
		if (state.TryGetField(tableIndex, field).IsOk)
		{
			return true;
		}
