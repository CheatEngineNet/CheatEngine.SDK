...
when a target is selected, a `TargetArchitectureObservation`; its `PointerSize` is Cheat Engine's configured pointer
size (`getPointerSize`), not the target bitness. It deliberately does not turn a legacy floating-point `getCEVersion`
result
...
follows `RuntimeInfo.TryDeriveTargetArchitecture`
(see [Runtime facts and target backends](#runtime-facts-and-target-backends)):
...
insufficient; their buffer-based forms are not applicable to the managed-hostfxr profile (audit A15-19). Fixture tests
verify these managed contracts; no test in this repository qualifies a live CE target,
...
An allocation is bound to the runtime identity and target incarnation that created it (`AllocatedRegion.Origin`). After
a
...
with a verified zero count is `NoMatches`; raw `nil` remains a distinct result, never a no-match inference. On the
pinned
...
separately named first-found opt-in, `TryFindFirstFoundWithinBounds`, is `[Experimental("CESDK5011")]` because its
result
...
manually. Releasing a session whose scan may still run first requests one cooperative stop (`terminateScan(false)`, then
a
...
`setPointerSize(2)` was accepted, and selecting the target again reset the value (spike C3 D3). The SDK therefore
reports
...
encoding of `PointerSize.TryReadLittleEndian`/`TryWriteLittleEndian` is an assumption of the local x86/x64 profile, not
a
...
never `getValue`. `AobBoundedScanResult` reports `HostScanElapsed` (from just before the SDK's first-scan call to the
end of
...
load (not a receipt, not SDK code): when 1 of 100 291 global matches lay in the scanned module, the range route took
about
...
13. Allocation ownership is consumed once; `TryAllocate` never yields a live address without an owner or a reported
    compensation, a release after detach reports `NotInvoked` and after re-enable is refused, and a released region
    never
