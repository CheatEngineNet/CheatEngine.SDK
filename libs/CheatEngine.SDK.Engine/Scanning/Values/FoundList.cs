...
public readonly struct FoundList : ICEObject<FoundList>, IEquatable<FoundList>
{
    ...
    {
        if (TryGetAddressText(zeroBasedIndex, out var text) && Address.TryParse(text, out address)) return true;
    ...
    private bool TryCallString(ReadOnlySpan<byte> method, int index, [NotNullWhen(true)] out string? value)
    {
        ...
        if (!_handle.TryCallMethod(state, method, 1, 1).IsOk || !StringMarshaller.TryRead(state, -1, out var text))
        {
            value = null;
            return false;
        }
