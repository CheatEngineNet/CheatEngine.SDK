...
internal static unsafe class LuaModuleLocator
{
	...
	{
		IntPtr handle = Locate(out bool fromSeam);
		if (handle == 0)
		{
			failure = fromSeam
	...
	internal static bool BindLocated(nint handle, bool counted, [NotNullWhen(false)] out string? failure)
	{
		...
		bool boundBefore = LuaApi.ModuleHandle == handle;
		bool bound = LuaApi.TryInitialize(handle, out string? bindFailure);
	...
	{
		delegate*<IntPtr> resolver = Resolver;
		...
		fromSeam = false;
		return LuaModule.TryGetLoaded(out IntPtr handle) ? handle : 0;
	}
}
