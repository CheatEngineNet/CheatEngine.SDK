...
public static unsafe class MainThread
{
	...
	{
		PluginContext context = RequireMainThreadContext();
		delegate* unmanaged[Stdcall]<void> pump = context.Exports.ProcessMessages;
		if (pump is null)
		{
			ThrowMissingSlot("ProcessMessages");
		}
	...
	public static bool CheckSynchronize(int timeoutMilliseconds)
	{
		...
		PluginContext context = RequireMainThreadContext();
		delegate* unmanaged[Stdcall]<int, Bool8> check = context.Exports.CheckSynchronize;
		if (check is null)
		{
			ThrowMissingSlot("CheckSynchronize");
		}
	...
	public static void Invoke<TState>(Action<TState> action, TState state)
	{
		...
		PluginContext context = PluginHost.RequireContext();
	...
	public static TResult Invoke<TState, TResult>(Func<TState, TResult> function, TState state)
	{
		...
		if (context.IsMainThread)
		{
			return MainThreadDispatcher.ExecuteInline(function, state);
		}
