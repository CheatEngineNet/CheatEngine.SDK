...
public static partial class LuaApi
{
	...
	{
		LuaApiBindResult result = Bind(moduleHandle, out string? failure);
		switch (result)
		{
	...
	{
		bool bound = Bind(moduleHandle, out string? message) == LuaApiBindResult.Bound;
	...
	public static IReadOnlyList<string> GetMissingExports(nint moduleHandle)
	{
		if (moduleHandle == 0)
		{
			throw new ArgumentException(NullModuleMessage, nameof(moduleHandle));
		}
