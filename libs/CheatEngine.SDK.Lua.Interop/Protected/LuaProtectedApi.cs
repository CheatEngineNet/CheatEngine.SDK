...
internal static unsafe partial class LuaProtectedApi
{
	...
	{
		BridgeBinding binding = ValidateInvocation(state, 2);
		int top = PushTableBeforeInputs(state, tableIndex, 2);
		try
		{
	...
	{
		BridgeBinding binding = ValidateInvocation(state, 1);
		int top = PushTableBeforeInputs(state, tableIndex, 1);
		try
		{
	...
	{
		int status = Invoke(state, LuaProtectedOperation.CreateReference, 1, GetPrivateReferenceKey(stableKey), 0, 0,
			0);
	...
	{
		BridgeBinding binding = ValidateInvocation(state, inputCount);
		...
		void* data, nuint size, nint first, nint second)
	{
		...
		LuaProtectedExports exports = binding.Exports;
		int status =
	...
	private static BridgeBinding ValidateInvocation(lua_State* state, int inputCount)
	{
		...
		BridgeBinding binding = EnsureLoaded();
	...
	{
		int top = LuaApi.lua_gettop(state);
		int tableValueIndex = GetValidTableIndex(tableIndex, top);
	...
	private static int GetValidTableIndex(int tableIndex, int top)
	{
		...
		if (tableIndex < LuaApi.LUA_REGISTRYINDEX)
		{
			long upvalue = (long) LuaApi.LUA_REGISTRYINDEX - tableIndex;
			if (upvalue is >= 1 and <= byte.MaxValue)
			{
				return tableIndex;
			}
		...
		long absoluteIndex = tableIndex > 0 ? tableIndex : (long) top + tableIndex + 1;
	...
	{
		IntPtr module = LuaApi.ModuleHandle;
		if (module == 0)
		{
			throw new InvalidOperationException("Bind LuaApi before invoking protected Lua operations.");
		}

		BridgeBinding? binding = Volatile.Read(ref s_binding);
	...
	private static void ValidateRequiredExports()
	{
		...
		try
		{
			...
			for (int i = 0; i < s_requiredBridgeExports.Length; i++)
			{
