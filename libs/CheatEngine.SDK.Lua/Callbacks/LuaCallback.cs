...
public abstract class LuaCallback : IDisposable
{
	...
	internal LuaCallback? Next
	{
		get;
		set;
	}

	internal LuaCallback? Previous
	{
		get;
		set;
	}
		...
		where TState : class
	{
		...
		try
		{
			...
			LuaStatus status = new(LuaProtectedApi.PushClosure(l,
			...
			if (!status.IsOk)
			{
				return state.KeepProtectedError(top, status);
			}

			status = TryCreateWrappedRef(state, l, top, out wrapped);
			if (!status.IsOk)
			{
				return status;
			}
			...
			out LuaRef? wrapped) // NOSONAR: native Lua stack access is required here.
	{
		...
		return state.TryCreateRef(out wrapped);
	}
	...
	public LuaStatus TryRegister(LuaState state, ReadOnlySpan<byte> globalName)
	{
		...
		if (TryPush(state))
		{
			return state.TrySetGlobal(globalName);
		}

		LuaStatus status = state.TryPushString(
	...
	{
		using LuaRuntimeOperation operation = state.IsNull ? default : LuaRuntime.EnterStateOperation(state);
		lock (LuaCallbackRegistry.Gate)
		{
	...
	private unsafe void ReleaseUnderGate(LuaState state) // NOSONAR: native Lua stack access is required here.
	{
		...
		finally
		{
			...
			{
				if (neutralized && _handle.IsAllocated)
				{
					_handle.Dispose();
				}
	...
	private static unsafe int Dispatch(lua_State* pointer) // NOSONAR: unmanaged callback entry point required by Lua.
	{
		...
		try
		{
			...
			try
			{
				IntPtr thunkAddress = (nint) lua_touserdata(pointer, lua_upvalueindex(2));
				if (thunkAddress == 0)
				{
					return LuaThunk.Fail(state, "callback thunk is unavailable"u8);
				}
