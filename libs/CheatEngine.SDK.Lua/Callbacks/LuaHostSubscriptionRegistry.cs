...
internal static class LuaHostSubscriptionRegistry
{
    ...
    internal static void Remove(LuaHostSubscription subscription)
    {
        lock (Gate)
        {
            if (!subscription.IsLinked) return;
    ...
    internal static void CloseCallbackAdmissionAndDrain()
    {
        ...
        lock (Gate)
        {
            ...
            for (var current = s_head; current is not null; current = current.Next) subscriptions.Add(current);
        }
    ...
    /// <summary>Closes admission and makes one LIFO unregister attempt for every owner while <paramref name="state" /> is valid.</summary>
    internal static void DetachAll(LuaState state)
    {
        ...
        while (TryTakeHead(out var subscription))
        {
