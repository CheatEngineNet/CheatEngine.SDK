...
public static class LuaThunk
{
		...
		where TState : class
	{
		...
		{
			void* upvalue = lua_touserdata(state.Pointer, lua_upvalueindex(1));
			if (upvalue is null)
			{
				value = null;
				return false;
	...
	public static unsafe int Fail(LuaState state, ReadOnlySpan<char> message)
	{
		try
		{
			...
			using Utf8Scratch utf8 = Utf8Scratch.Encode(message, scratch);
	...
	public static int FailBadArgument(LuaState state, int argument, ReadOnlySpan<byte> expected)
	{
		...
		if (!argument.TryFormat(message[length..], out int digits, default, CultureInfo.InvariantCulture))
		{
			digits = 0;
		}
