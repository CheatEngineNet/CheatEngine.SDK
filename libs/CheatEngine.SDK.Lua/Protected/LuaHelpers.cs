...
internal static unsafe class LuaHelpers
{
	...
	{
		if (lua_rawgetp(l, LUA_REGISTRYINDEX, Key(helper)) == LUA_TFUNCTION)
		{
			return LuaStatus.Ok;
		}
	...
	{
		LuaStatus status = Install(l);
		if (!status.IsOk)
		{
			return status;
		}

		int type = lua_rawgetp(l, LUA_REGISTRYINDEX, Key(helper));
	...
	internal static LuaStatus Install(lua_State* L)
	{
		...
		fixed (byte* name = "=CheatEngine.SDK.Lua"u8)
		...
		{
			return new LuaStatus(loadStatus);
		}

		PushSentinel(L);
		int runStatus = lua_pcallk(L, 1, Count, 0, 0, null);
		if (runStatus != LUA_OK)
		{
			return new LuaStatus(runStatus);
		}

		// The last result is on top; rawsetp pops the top each time, so the keys are assigned in reverse.
		for (int i = Count - 1; i >= 0; i--)
		{
			LuaStatus status = new(LuaProtectedApi.RawSetP(L, LUA_REGISTRYINDEX, SKeyBlock + i));
			if (!status.IsOk)
			{
				return new LuaState((nint) L).KeepProtectedError(top, status);
			}
		}
