...
public readonly record struct LuaRegistrationResult
{
	...
	public LuaRegistrationResultKind Kind
	{
		get;
	}

	/// <summary>Gets the collision or protected-operation failure when <see cref="IsSuccess" /> is <see langword="false" />.</summary>
	public LuaRegistrationFailure? Failure
	{
		get;
	}

	/// <summary>
	///     Gets the compensation outcome after a publication failure, otherwise
	///     <see cref="LuaRegistrationReleaseKind.NotAttempted" />.
	/// </summary>
	public LuaRegistrationReleaseOutcome Rollback
	{
		get;
	}

	/// <summary>
	///     Gets the complete successful lease, or a residual lease after failed compensation. Dispose every non-null
	///     lease.
	/// </summary>
	public LuaRegistrationLease? Lease
	{
		get;
	}
