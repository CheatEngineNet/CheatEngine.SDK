...
public static unsafe class LuaRuntime
{
    ...
    {
        if (!TryAcquireOperation(out var operation))
        {
            state = default;
            return false;
        }
    ...
    internal static void ExitOperation()
    {
        ...
        lock (SOperationGate)
        {
            if (--s_activeOperations == 0) SOperationsDrained.Set();
        }
    }
    ...
    private static bool TryEnterOperation()
    {
        lock (SOperationGate)
        {
            if (s_services is null || !s_acceptOperations) return false;
    ...
    private static void IncrementActiveOperation()
    {
        ...
        t_operationDepth++;
        if (s_activeOperations == 1) SOperationsDrained.Reset();
    }
    ...
    private static void OpenOperationAdmission()
    {
        lock (SOperationGate)
        {
            if (s_services is not null) s_acceptOperations = true;
        }
    }
