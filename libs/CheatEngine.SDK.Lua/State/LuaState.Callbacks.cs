...
public readonly unsafe partial struct LuaState
{
    ...
    private static int DispatchGeneratedFunction(lua_State* pointer)
    {
        ...
        try
        {
            var required = lua_tointegerx(pointer, lua_upvalueindex(4), null) != 0;
            if (required)
            {
                var attachEpoch = checked((int)lua_tointegerx(pointer, lua_upvalueindex(2), null));
                var stateGeneration = checked((int)lua_tointegerx(pointer, lua_upvalueindex(3), null));
    ...
    private static unsafe int InvokeGeneratedThunk(lua_State* pointer, LuaState state)
    {
        var thunkAddress = (nint)lua_touserdata(pointer, lua_upvalueindex(1));
        if (thunkAddress == 0) return LuaThunk.Fail(state, "Lua function thunk is unavailable"u8);
