...
public readonly unsafe partial struct LuaState
{
	...
	public LuaStatus TryLoad(ReadOnlySpan<byte> source, ReadOnlySpan<byte> chunkName = default)
	{
		...
		fixed (byte* mode = "t"u8)
		{
			...
			int length = Math.Min(chunkName.Length, name.Length - 1);
	...
	{
		LuaStatus status = TryLoad(source, chunkName);
	...
	{
		int top = Top;
		LuaStatus status = LuaHelpers.Push(Pointer, LuaHelper.GetGlobal);
		if (!status.IsOk)
		{
			return status;
		}
	...
	public LuaStatus TrySetGlobal(ReadOnlySpan<byte> name)
	{
		...
		LuaStatus status = LuaHelpers.Push(Pointer, LuaHelper.SetGlobal);
		if (!status.IsOk)
		{
	...
	{
		int top = Top;
		LuaStatus status = LuaHelpers.Push(Pointer, LuaHelper.Index);
		if (!status.IsOk)
		{
			return status;
		}
	...
	public LuaStatus TrySetField(int index, ReadOnlySpan<byte> key)
	{
		...
		LuaStatus status = LuaHelpers.Push(Pointer, LuaHelper.NewIndex);
		if (!status.IsOk)
		{
			lua_rotate(Pointer, -2, 1);
			lua_settop(Pointer, -2);
	...
	{
		LuaStatus status = LuaHelpers.Push(Pointer, LuaHelper.Length);
		if (!status.IsOk)
		{
			return status;
		}
	...
	{
		LuaStatus status = LuaHelpers.Push(Pointer, LuaHelper.ToString);
		if (!status.IsOk)
		{
			return status;
		}
	...
	{
		result = false;
		LuaStatus status = LuaHelpers.Push(Pointer, LuaHelper.Compare);
		if (!status.IsOk)
		{
			return status;
		}
	...
	public LuaStatus TryNext(int index, out bool hasNext)
	{
		// [.. k] -> [.. k h] -> [.. k h t] -> [.. h t k]
		hasNext = false;
		LuaStatus status = LuaHelpers.Push(Pointer, LuaHelper.Next);
		if (!status.IsOk)
		{
			lua_rotate(Pointer, -2, 1);
			lua_settop(Pointer, -2);
