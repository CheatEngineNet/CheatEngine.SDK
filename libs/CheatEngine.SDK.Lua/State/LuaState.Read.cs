...
public readonly unsafe partial struct LuaState
{
	...
	public bool TryReadUtf8(int index, out ReadOnlySpan<byte> utf8)
	{
		...
		nuint length;
		byte* bytes = lua_tolstring(Pointer, index, &length);
	...
	{
		if (!TryReadUtf8(index, out ReadOnlySpan<byte> utf8) || !utf8.TryCopyTo(destination))
		{
			written = 0;
			return false;
		}
	...
	{
		if (!TryReadUtf8(index, out ReadOnlySpan<byte> utf8))
		{
			value = null;
			return false;
		}
