...
public readonly partial struct LuaState
{
	...
	{
		using LuaRuntimeOperation operation = LuaRuntime.EnterStateOperation(this);
		LuaStateIdentity identity = LuaRuntime.CurrentStateIdentity;
		CheckProtectedResult(LuaReferences.Create(this, out int reference));
	...
	internal LuaStatus TryCreateRef(out LuaRef? reference)
	{
		using LuaRuntimeOperation operation = LuaRuntime.EnterStateOperation(this);
		LuaStateIdentity identity = LuaRuntime.CurrentStateIdentity;
		LuaStatus status = LuaReferences.Create(this, out int slot);
	...
	public bool TryPushRef(LuaRef reference)
	{
		...
		{
			if (reference is null || !reference.TryGetCurrent(out int slot))
			{
				return false;
			}
