...
namespace CESDK.SourceGenerators.EntryPoint.Emit;
...
/// <remarks>
///     <para>
///         Rules of the emitted text: every type name is <c>global::</c>-qualified (inside <c>namespace CESDK</c> the
///         simple
///         name <c>CESDK</c> binds to the entry-point class, not to the namespace); no <see langword="unsafe" /> code, so
///         the
///         file compiles in a project that forbids it; no dependency on the consumer's usings, nullable context or
///         implicit
///         usings; nothing newer than C# 11 (<c>file</c> types, <c>u8</c> literals, static abstract interface members).
///     </para>
///     <para>
...
///         <c>CESDK.Abi.Managed.ManagedEntryPoint</c>
...
internal static class BootstrapEmitter
{
    ...
    public const string HintName = "CESDK.EntryPoint.g.cs";
    ...
    public static SourceText Emit(BootstrapModel model)
    {
        ...
        var factoryName = ChooseFactoryName(model.FullyQualifiedTypeName);
    ...
    {
        var collides = fullyQualifiedPluginTypeName.StartsWith(QualifiedFactoryName, StringComparison.Ordinal)
    ...
    private static void WriteEntryPoint(SourceWriter writer, string factoryName)
    {
        ...
        writer.OpenBlock();
        writer.Write("return global::CESDK.Hosting.Bootstrap.PluginHost.InitializeManaged<");
    ...
    private static void WriteFactory(SourceWriter writer, BootstrapModel model, string factoryName)
    {
        ...
        writer.Write(factoryName);
        writer.WriteLine(" : global::CESDK.Hosting.Plugin.IPluginFactory");
        writer.OpenBlock();

        writer.Write("public static global::CESDK.Hosting.Plugin.CheatEnginePlugin Create() => new ");
