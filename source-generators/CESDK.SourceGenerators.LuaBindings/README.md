...
the public API of [`CESDK.Lua`](../../libs/CESDK.Lua/README.md).
...
1. Two pipelines find the methods that carry `CESDK.Annotations.Lua.LuaFunctionAttribute` or
   `CESDK.Annotations.Lua.LuaGlobalAttribute` in the current compilation.
...
4. The emitters write each file against the `CESDK.Lua` API, with every type name `global::`-qualified. The function
...
Registration takes the address of each thunk, so the generator needs `AllowUnsafeBlocks`. The `CESDK` package sets it
