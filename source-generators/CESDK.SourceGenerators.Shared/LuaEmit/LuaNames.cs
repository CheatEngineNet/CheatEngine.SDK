...
///     never a run-time UTF-8 transcode, so it never reaches <c>CESDK.Lua.Text.Utf8Scratch</c>'s <c>stackalloc</c>/pool
///     path (that path is for the run-time <em>content</em> of a <see cref="LuaValueKind.String" /> value, an unrelated,
///     already length-agnostic concern of the <c>CESDK.Lua</c> layer).
/// </remarks>
