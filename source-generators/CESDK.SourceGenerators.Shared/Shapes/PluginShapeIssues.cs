...
///     must compile without an object initializer and must yield a <c>CESDK.Hosting.Plugin.CheatEnginePlugin</c>.
/// </summary>
/// <remarks>
///     The single source of truth for both consumers of <see cref="PluginShape" />: the
///     <c>CESDK.SourceGenerators.EntryPoint</c> generator only tests <see cref="None" /> to decide whether to stay
///     silent, and analyzer rule CESDK0001 (<c>CESDK.Analyzers</c>) reports one diagnostic per flag that is set, in a
...
internal enum PluginShapeIssues
{
    ...
    /// <summary>
    ///     No base class is named <c>CheatEnginePlugin</c> in namespace <c>CESDK.Hosting.Plugin</c>. Checked
    ///     structurally (name and namespace of each symbol in the base-type chain), not by resolving
    ///     <c>CESDK.Hosting.Plugin.CheatEnginePlugin</c> and comparing symbols: the decision then depends on nothing but
