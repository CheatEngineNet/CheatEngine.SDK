...
internal static class LuaFunctionFileEmitter
{
    ...
    public static SourceText Emit(LuaFunctionTableModel table)
    {
        ...
        foreach (var thunk in table.Thunks)
        {
            writer.WriteLine();
    ...
    {
        var ids = CollectDeclaredDiagnosticIds(thunks);
    ...
    private static string CollectDeclaredDiagnosticIds(EquatableArray<LuaThunkModel> thunks)
    {
        ...
        {
            if (thunk.DeclaredDiagnosticIds.Length == 0) continue;

            foreach (var id in thunk.DeclaredDiagnosticIds.Split(IdSeparator, StringSplitOptions.None))
            {
