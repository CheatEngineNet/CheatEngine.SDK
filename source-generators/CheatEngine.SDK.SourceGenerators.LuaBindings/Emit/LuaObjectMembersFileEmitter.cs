...
internal static class LuaObjectMembersFileEmitter
{
	...
	public static SourceText Emit(LuaObjectMembersTableModel table)
	{
		...
		foreach (LuaObjectMethodModel method in table.Methods)
		{
		...
		foreach (LuaObjectPropertyModel property in table.Properties)
		{
			writer.WriteLine(GeneratedCodeAttribute);
	...
	private static void WriteTryExceptionCatch(SourceWriter writer, LuaObjectMethodModel model)
	{
		...
		{
			WriteDefaultResult(writer, result);
		}
	...
	{
		int requiredSlots = Math.Max(2 + model.Arguments.Length, 1 + model.ResultCount);
		if (requiredSlots > StackCheckThreshold)
		{
			WriteStackCheck(writer, model, requiredSlots);
		}
	...
	private static void WriteStackCheck(SourceWriter writer, LuaObjectMethodModel model, int requiredSlots)
	{
		...
		if (model.Form == LuaCallForm.Try)
		{
			for (int i = 1; i < model.Results.Length; i++)
			{
				WriteDefaultResult(writer, model.Results[i]);
			}
	...
	private static void WriteTryResults(SourceWriter writer, LuaObjectMethodModel model)
	{
		for (int i = 0; i < model.Results.Length; i++)
		{
			...
			writer.WriteLine("))");
			writer.OpenBlock();
			for (int other = 0; other < model.Results.Length; other++)
			{
				if (other != i)
				{
	...
	private static void EmitProperty(SourceWriter writer, LuaObjectPropertyModel model)
	{
		...
		if (model.HasGetter)
		{
			EmitGetter(writer, model);
		}

		if (model.HasSetter)
		{
			EmitSetter(writer, model);
		}
