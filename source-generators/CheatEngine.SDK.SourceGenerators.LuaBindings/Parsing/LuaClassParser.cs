...
internal static class LuaClassParser
{
	...
	{
		if (type.TypeKind != TypeKind.Struct || type.IsGenericType || type.IsRecord || type.IsRefLikeType)
		{
			return false;
		}

		if (ContainingTypeShape.Inspect(type, cancellationToken) != ContainingTypeIssues.None)
		{
			return false;
		}

		if (!IsReadOnlyStruct(type, cancellationToken))
		{
			return false;
		}
	...
	internal static bool IsGeneratedHandle(INamedTypeSymbol type, Compilation compilation,
		CancellationToken cancellationToken)
	{
		if (!IsBorrowedHandleShape(type, compilation, cancellationToken))
		{
			return false;
		}
