...
internal static class ContainingTypeShape
{
	...
	{
		ContainingTypeIssues issues = ContainingTypeIssues.None;
		for (INamedTypeSymbol? current = type; current is not null; current = current.ContainingType)
		{
			...
			{
				issues |= ContainingTypeIssues.Generic;
			}

			if (current.IsFileLocal)
			{
				issues |= ContainingTypeIssues.FileLocal;
			}

			if (!IsPartial(current, cancellationToken))
			{
				issues |= ContainingTypeIssues.NotPartial;
			}
		}
