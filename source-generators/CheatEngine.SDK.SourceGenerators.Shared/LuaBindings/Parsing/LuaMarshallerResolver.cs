...
internal static class LuaMarshallerResolver
{
    ...
    {
        if (compilation is not null && !compilation.IsSymbolAccessibleWithin(candidate, bindingType)) return false;

        var closedContract = contract.Construct(valueType);
        ...
        INamedTypeSymbol candidate, INamedTypeSymbol closedContract, string name)
    {
        ...
        foreach (var member in closedContract.GetMembers(name))
    ...
    private static bool HasMatchingSignature(IMethodSymbol candidate, IMethodSymbol required)
    {
        ...
        {
            var actual = candidate.Parameters[i];
            var expected = required.Parameters[i];
