...
| `CheatEngine.SDK.SourceGenerators.Shared.Shapes`                                                              | `PluginShape.Inspect` and `PluginShapeIssues`: one flag per reason a `[CheatEnginePlugin]` class cannot be constructed by the generated entry point. The base class `CheatEngine.SDK.Hosting.Plugin.CheatEnginePlugin` is matched by name and namespace, and any constructor callable with no arguments counts. | `EntryPoint` emits only when the result is `None`. `CheatEngine.SDK.Analyzers` rule `CESDK0001` reports one diagnostic per flag.                                                                   |
...
- Every name the generators write into generated code or look symbols up by denotes a real type: the metadata names
  against `CheatEngine.SDK.Annotations` and `CheatEngine.SDK.Hosting` (`AnnotationsMetadataNamesTests`), and the `global::CheatEngine.SDK.Lua` names
  and the `LuaState` recognition against `CheatEngine.SDK.Lua` (`LuaApiNamesTests`). A namespace move that leaves a string behind
