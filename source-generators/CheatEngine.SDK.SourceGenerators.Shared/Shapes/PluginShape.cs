...
internal static class PluginShape
{
		...
		out IMethodSymbol? parameterlessConstructor)
	{
		...
		PluginShapeIssues issues = string.IsNullOrWhiteSpace(displayName)
			? PluginShapeIssues.InvalidName
			: PluginShapeIssues.None;
		...
		issues |= InspectTypeShape(type, pluginBase, obsoleteAttribute);
	...
	private static PluginShapeIssues InspectTypeShape(
		INamedTypeSymbol type,
		INamedTypeSymbol? pluginBase,
		INamedTypeSymbol? obsoleteAttribute)
	{
		PluginShapeIssues issues = PluginShapeIssues.None;

		if (type.IsAbstract)
		{
			issues |= PluginShapeIssues.Abstract;
		}

		if (type.Arity > 0)
		{
			issues |= PluginShapeIssues.Generic;
		}

		if (type.ContainingType is { IsGenericType: true })
		{
			issues |= PluginShapeIssues.NestedInGeneric;
		}

		if (!DerivesFromPluginBase(type, pluginBase))
		{
			issues |= PluginShapeIssues.NotDerivedFromPluginBase;
		}

		for (INamedTypeSymbol? current = type; current is not null; current = current.ContainingType)
		{
			if (!IsAssemblyWide(current.DeclaredAccessibility))
			{
				issues |= PluginShapeIssues.Inaccessible;
			}

			if (current.IsFileLocal)
			{
				issues |= PluginShapeIssues.FileLocal;
			}

			// 'new global::Outer.Plugin()' names every type of the chain.
			if (IsObsoleteError(current, obsoleteAttribute))
			{
				issues |= PluginShapeIssues.ObsoleteError;
			}
		}

		if (IsOrIsNestedInEntryPointType(type))
		{
			issues |= PluginShapeIssues.ReservedEntryPointName;
		}
	...
	private static string ReadDisplayName(AttributeData? attribute)
	{
		...
		TypedConstant name = attribute.ConstructorArguments[0];
		...
		out IMethodSymbol? parameterlessConstructor)
	{
		...
		PluginShapeIssues issues = parameterlessConstructor switch
		{
		...
		{
			if (IsObsoleteError(parameterlessConstructor, obsoleteAttribute))
			{
				issues |= PluginShapeIssues.ObsoleteError;
			}
		...
		if (!setsRequiredMembers && HasRequiredMembers(type))
		{
			issues |= PluginShapeIssues.RequiredMembers;
		}
