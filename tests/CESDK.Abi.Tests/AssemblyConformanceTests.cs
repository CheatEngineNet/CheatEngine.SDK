...
/// <summary>
///     Assembly-wide gates. They exist so that a structure or enumeration added to <c>CESDK.Abi</c> without a row in
///     the expected-size table, or with a shape the ABI does not allow (see <see cref="AbiShape" />: forbidden field
///     types at any depth, function pointers that are managed, not <c>Stdcall</c>, or carry a forbidden type in their
///     signature), fails the test run instead of slipping through.
/// </summary>
/// <remarks>
...
public sealed class AssemblyConformanceTests
{
    ...
    [Fact]
    public void Assembly_references_no_other_cesdk_assembly()
    {
        var cesdkReferences = AbiAssembly.GetReferencedAssemblies()
            .Select(static name => name.Name ?? string.Empty)
            .Where(static name => name.StartsWith("CESDK", StringComparison.Ordinal))
            .ToArray();

        Assert.Empty(cesdkReferences);
    }
