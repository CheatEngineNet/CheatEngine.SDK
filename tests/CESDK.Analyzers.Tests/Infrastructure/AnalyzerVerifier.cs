    ...
    where TAnalyzer : DiagnosticAnalyzer, new()
{
    ...
    {
        return RunAsync(new CesdkAnalyzerTest<TAnalyzer>(), sources, expected);
    }
    ...
    {
        CesdkAnalyzerTest<TAnalyzer> test = new();
    ...
    /// <summary>Verifies a project that does not reference CESDK at all: the analyzers must stay out of the way.</summary>
    public static Task VerifyWithoutCesdkAsync(string source, params DiagnosticResult[] expected)
    {
        return RunAsync(new CesdkAnalyzerTest<TAnalyzer>(false), [("Test0.cs", source)], expected);
    }

    private static Task RunAsync(CesdkAnalyzerTest<TAnalyzer> test, (string FileName, string Source)[] sources,
