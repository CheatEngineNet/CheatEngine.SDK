...
///     </para>
///     <para>
///         Same algorithm as <c>LocalFrameworkReferences</c> of <c>CESDK.SourceGenerators.EntryPoint.Tests</c>; the two
///         copies are candidates for one file in <c>tests/CESDK.Tests.Shared</c>.
///     </para>
/// </remarks>
internal static class LocalFrameworkReferences
{
    ...
    public static ReferenceAssemblies WithoutPackages { get; } = new(TargetFramework);
    ...
    public static ImmutableArray<MetadataReference> FromTargetingPack()
    {
        ...
        {
            var candidate = Path.Combine(pack, "ref", TargetFramework);
