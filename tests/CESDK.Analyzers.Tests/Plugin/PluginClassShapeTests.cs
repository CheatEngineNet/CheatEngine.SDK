...
public sealed class PluginClassShapeTests
{
    ...
    {
        await Verifier.VerifyAsync("""
                                   [CESDK.Annotations.Plugin.CheatEnginePlugin("Demo")]
                                   public sealed class DemoPlugin : CESDK.Hosting.Plugin.CheatEnginePlugin
                                   {
                                       protected override void OnEnable() { }
                                       protected override void OnDisable() { }
                                   }
    ...
    public async Task Attribute_through_an_alias_is_recognised()
    {
        await Verifier.VerifyAsync(
            """
            using CESDK.Hosting.Plugin;
            using Plugin = CESDK.Annotations.Plugin.CheatEnginePluginAttribute;
    ...
    public async Task Attribute_with_a_fully_qualified_name_is_recognised()
    {
        await Verifier.VerifyAsync(
            ...
            [global::CESDK.Annotations.Plugin.CheatEnginePluginAttribute("Demo")]
            public abstract class {|#0:DemoPlugin|} : global::CESDK.Hosting.Plugin.CheatEnginePlugin
            {
            }
            """,
            Problem(0, "DemoPlugin", PluginShapeIssues.Abstract));
    ...
    [Fact]
    public async Task Project_without_a_cesdk_reference_is_not_analysed()
    {
        await Verifier.VerifyWithoutCesdkAsync("""
                                               using System;
