...
namespace CESDK.Engine.Tests.Objects;
...
public sealed unsafe class CEObjectTests
{
    ...
    public void TryRead_decodes_a_full_userdata_whose_first_field_is_the_object_pointer()
    {
        ...
        var probe = FakeHost.CreateObject(L, "Probe");
        probe.Push(L);
        Assert.True(L.IsUserdata(-1));
        Assert.True(CEObject.TryRead(L, -1, out var fromPusher));
        Assert.Equal(probe, fromPusher);

        // A userdata made by hand with the same layout, larger than a pointer.
        var block = L.NewUserdata(32);
        *(nint*)block = 0x7777_0000;
        Assert.True(CEObject.TryRead(L, -1, out var fromBlock));
    ...
    public void TryRead_refuses_values_that_are_not_host_objects()
    {
        ...
        var zeroed = L.NewUserdata((nuint)sizeof(nint));
    ...
    public void Push_goes_through_the_host_pusher_and_a_null_handle_pushes_nil()
    {
        ...
        var probe = FakeHost.CreateObject(L, "Probe");
        var pushes = FakeHost.PusherCalls;
        probe.Push(L);
        ...
        Assert.True(CEObject.TryRead(L, -2, out var back));
    ...
    public void TryGetProperty_leaves_only_the_value_on_the_stack()
    {
        ...
        var L = scope.State;
        using LuaFrame frame = new(L);
        var probe = FakeHost.CreateObject(L, "Probe", "o.props.Count = 3; o.props.Name = 'probe'");
    ...
    public void A_raising_getter_or_setter_is_a_status_with_one_error_value()
    {
        ...
        var status = probe.TryGetProperty(L, "Bad"u8);
    ...
    public void Indexed_access_passes_the_zero_based_index_through_unchanged()
    {
        ...
        var L = scope.State;
        using LuaFrame frame = new(L);
        var probe = FakeHost.CreateObject(L, "Probe", "o.items = { 'first', 'second' }");
    ...
    public void TryPushMethod_pushes_an_instance_bound_function_that_is_called_without_self()
    {
        ...
        var L = scope.State;
        using LuaFrame frame = new(L);
        var probe = FakeHost.CreateObject(L, "Probe", "o.props.Count = 5");
    ...
    public void TryPushMethod_reports_a_member_that_is_not_a_function_by_name_and_type()
    {
        ...
        var status = probe.TryPushMethod(L, "notAMethod"u8);
        Assert.Equal(LuaStatus.RuntimeError, status);
    ...
    public void TryCallMethod_replaces_the_arguments_by_the_results()
    {
        ...
        var L = scope.State;
        using LuaFrame frame = new(L);
        var probe = FakeHost.CreateObject(L, "Probe", "o.props.Count = 9");
    ...
    public void TryCallMethod_replaces_the_arguments_by_one_error_value_on_failure()
    {
        ...
        L.PushInteger(1);
        L.PushInteger(2);
        var status = probe.TryCallMethod(L, "raise"u8, 2, 1);
        Assert.Equal(LuaStatus.RuntimeError, status);
    ...
    public void Typed_members_acquire_the_state_read_through_the_marshaller_and_restore_the_stack()
    {
        ...
        Assert.True(probe.TryGetProperty<StringMarshaller, string>("Name"u8, out var name));
        Assert.Equal("probe", name);
        Assert.True(probe.TryGetProperty<Address, Address>("Result"u8, out var number));
        Assert.Equal(0x400000UL, number.Value);
        Assert.True(probe.TryGetProperty<Address, Address>("Text"u8, out var text));
        Assert.Equal(0x401000UL, text.Value);
        Assert.True(probe.TryGetProperty<EnumMarshaller<VariableType>, VariableType>("Count"u8, out var asEnum));
        Assert.Equal(VariableType.Qword, asEnum);

        Assert.False(probe.TryGetProperty<Int32Marshaller, int>("Name"u8, out var notAnInteger));
        ...
        Assert.False(probe.TryGetProperty<StringMarshaller, string>("Missing"u8, out var missing));
        Assert.Null(missing);
        ...
        Assert.True(probe.TryCallMethod<Int32Marshaller, int>("getCount"u8, out var viaMethod));
    ...
    public void An_object_travels_as_an_argument_and_comes_back_as_the_same_handle()
    {
        ...
        var L = scope.State;
        var probe = FakeHost.CreateObject(L, "Probe");
        var other = FakeHost.CreateObject(L, "Probe");
        ...
        Assert.True(probe.TryGetProperty<CEObject, CEObject>("Other"u8, out var back));
        Assert.Equal(other, back);
        Assert.NotEqual(probe, back);
        Assert.False(probe.TryGetProperty<CEObject, CEObject>("Missing"u8, out var none));
    ...
    public void A_method_returning_hexadecimal_text_is_read_by_the_address_reader_with_a_zero_based_index()
    {
        ...
        var L = scope.State;
        using LuaFrame frame = new(L);
        var list = FakeHost.CreateObject(L, "Probe", "o.addresses = { 0x400000, 0x7FF6A1B2C3D4 }");

        L.PushInteger(0);
        Assert.True(list.TryCallMethod(L, "getAddress"u8, 1, 1).IsOk);
        Assert.Equal("00400000", EngineTest.ReadString(L, -1));
        Assert.True(Address.TryRead(L, -1, out var first));
        ...
        Assert.True(Address.TryRead(L, -1, out var second));
