...
public sealed class AddressLuaTests
{
    ...
    public void A_number_is_read_by_bit_reinterpretation()
    {
        ...
        Assert.True(Address.TryRead(L, 1, out var low));
        Assert.Equal(0x400000UL, low.Value);
        Assert.True(Address.TryRead(L, 2, out var high));
        Assert.Equal(ulong.MaxValue, high.Value);
        Assert.True(Address.TryRead(L, 3, out var integralFloat));
        Assert.Equal(0x401000UL, integralFloat.Value);
        Assert.False(Address.TryRead(L, 4, out var fraction));
    ...
    public void A_string_is_read_as_hexadecimal_never_as_decimal()
    {
        ...
        Assert.True(Address.TryRead(L, 1, out var padded));
        Assert.Equal(0x400000UL, padded.Value);
        Assert.True(Address.TryRead(L, 2, out var prefixed));
        Assert.Equal(0x7FF6A1B2C3D4UL, prefixed.Value);
        Assert.True(Address.TryRead(L, 3, out var ten));
    ...
    public void Push_and_read_round_trip_through_the_marshaller_contract()
    {
        ...
        Assert.True(RoundTrip<Address, Address>(L, out var back));
    ...
    public void Reading_either_form_allocates_nothing()
    {
        ...
        {
            if (!Address.TryRead(L, 1, out var number)) Assert.Fail("number read failed");
            if (!Address.TryRead(L, 2, out var text)) Assert.Fail("text read failed");
