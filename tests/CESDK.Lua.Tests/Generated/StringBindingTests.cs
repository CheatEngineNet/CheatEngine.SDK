...
public sealed class StringBindingTests
{
    ...
    public void Copy_out_shape_reads_the_bytes_and_leaves_the_stack_balanced()
    {
        ...
        Assert.True(StringBindings.TryReadString(0x1000, 64, buffer, out var written));
    ...
    public void Copy_out_shape_fails_cleanly_for_nil_a_too_small_buffer_a_raise_and_a_missing_global()
    {
        ...
        Assert.False(StringBindings.TryReadString(0x1000, 64, buffer, out var written)); // no global yet
    ...
    public void Copied_bytes_survive_collection_and_reuse_of_the_lua_string_memory()
    {
        ...
        Assert.True(StringBindings.TryReadString(0x2000, 4096, copy, out var written));
        Assert.Equal(4096, written);
        LuaTest.Run(L, "return churn()"u8, 1);
        Assert.True(L.TryReadInteger(-1, out var kept));
    ...
    public void String_shape_decodes_the_text_and_is_null_on_failure()
    {
        ...
        Assert.True(StringBindings.TryReadString(0x1000, 64, out var text));
        Assert.Equal("Cheat Engine", text);
        Assert.False(StringBindings.TryReadString(0x3000, 64, out var missing));
        Assert.Null(missing);
        Assert.Equal(0, L.Top);
    }
    ...
    public void Copy_out_shape_allocates_nothing()
    {
        ...
        {
            if (!StringBindings.TryReadString(0x1000, 64, buffer, out var written) || written != 12)
