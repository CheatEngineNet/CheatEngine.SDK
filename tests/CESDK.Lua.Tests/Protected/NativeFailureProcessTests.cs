...
namespace CESDK.Lua.Tests.FailureBoundaries;
...
public sealed class NativeFailureProcessTests
{
    ...
    public async Task Allocation_failures_return_through_the_native_boundary()
    {
        ...
        var probe = Path.Combine(baseDirectory, "CESDK.Lua.FailureProbe.dll");
        var runtimeConfig = Path.Combine(baseDirectory, "CESDK.Lua.Tests.runtimeconfig.json");
        var depsFile = Path.Combine(baseDirectory, "CESDK.Lua.Tests.deps.json");
