...
internal static class StepAssert
{
    ...
    /// <summary>
    ///     Every CESDK-named step ran, produced something, and produced only <c>Cached</c>/<c>Unchanged</c> values; no
    ///     source output was re-executed. Also fails when a step exists that <see cref="EngineApiTrackingNames.All" />
    ///     does not know, so a new step cannot slip past the gate.
    /// </summary>
    public static void NothingWasRecomputed(GeneratorRunResult result)
    {
        string[] trackedCesdkSteps =
            [.. result.TrackedSteps.Keys.Where(TrackingNames.IsCesdkStep).Order(StringComparer.Ordinal)];
        Assert.Equal(EngineApiTrackingNames.All.Order(StringComparer.Ordinal), trackedCesdkSteps,
    ...
    /// <summary>Reasons of every source output; fails when there is none.</summary>
