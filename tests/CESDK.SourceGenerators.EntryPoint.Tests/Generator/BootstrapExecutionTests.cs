...
public sealed class BootstrapExecutionTests(RoslynFixture roslyn) : IClassFixture<RoslynFixture>
{
    ...
    {
        var run = roslyn.Run($$"""
                               [CESDK.Annotations.Plugin.CheatEnginePlugin("Throws")]
                               public sealed class ThrowingPlugin : CESDK.Hosting.Plugin.CheatEnginePlugin
                               {
    ...
    public void Entry_point_nested_internal_plugin_is_constructed()
    {
        var run = roslyn.Run($$"""
                               namespace Demo
                               {
                                   ...
                                   {
                                       [CESDK.Annotations.Plugin.CheatEnginePlugin("Nested \u00E9")]
