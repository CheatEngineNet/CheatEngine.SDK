...
{
    private const string Usings = "using CESDK.Annotations.Plugin; using CESDK.Hosting.Plugin;\n";
    ...
    public static TheoryData<string, string> InvalidShapes
    {
        get
        {
            ...
            foreach (var (shape, declaration) in ClassShapeRejections()) data.Add(shape, declaration);
            foreach (var (shape, declaration) in AccessibilityAndBaseRejections()) data.Add(shape, declaration);
            foreach (var (shape, declaration) in ConstructorRejections()) data.Add(shape, declaration);
            foreach (var (shape, declaration) in NameAndEntryPointRejections()) data.Add(shape, declaration);
            return data;
        }
    }
    ...
    {
        var run = roslyn.Run("namespace Demo; public sealed class NotAPlugin { }");
    ...
    public void Generator_attribute_with_the_same_simple_name_from_another_namespace_emits_nothing()
    {
        var run = roslyn.Run($$"""
                               ...
                               public sealed class P : CESDK.Hosting.Plugin.CheatEnginePlugin {{Body}}
                               """);
    ...
    {
        var run = roslyn.Run(Usings + declaration);
    ...
    public void Generator_build_property_false_emits_nothing(string value)
    {
        var run = RoslynFixture.Run(
            ...
            TestAnalyzerConfigOptionsProvider.WithBuildProperty("CesdkGenerateEntryPoint", value));
    ...
    public void Generator_unrelated_build_property_is_ignored()
    {
        var run = RoslynFixture.Run(
            roslyn.CreateCompilation(PluginSources.Nominal),
            TestAnalyzerConfigOptionsProvider.WithBuildProperty("CesdkSomethingElse", "false"));
