...
public sealed class NominalOutputTests(RoslynFixture roslyn) : IClassFixture<RoslynFixture>
{
    ...
    [Fact]
    public void ManagedEntryPointNames_matches_CESDK_Abi_ManagedEntryPoint()
    {
        // Parity check for the generator-side identity constants BootstrapEmitter writes (source-generators/CESDK.SourceGenerators.Shared/
        // ManagedEntryPointNames.cs) against their net10.0-side source of truth: the netstandard2.0 Roslyn component
        // cannot reference CESDK.Abi directly, so this is what keeps the two copies from drifting.
