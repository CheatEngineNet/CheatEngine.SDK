...
namespace CESDK.SourceGenerators.LuaBindings.Tests.Generator;
...
public sealed class IncrementalityTests(RoslynFixture roslyn) : IClassFixture<RoslynFixture>
{
    ...
    public void Pipeline_unattributed_member_added_to_a_binding_type_recomputes_nothing()
    {
        ...
        var original = compilation.SyntaxTrees.First();
        var edited = BindingSources.Functions.Replace(
            "[LuaFunction(\"add\")]",
    ...
    public void Pipeline_function_attribute_argument_edited_reruns_the_function_output_only()
    {
        ...
        var original = compilation.SyntaxTrees.First();
        var edited = BindingSources.Functions.Replace("[LuaFunction(\"add\")]", "[LuaFunction(\"plus\")]",
            StringComparison.Ordinal);
        var second = GeneratorRun.Execute(first.Driver,
            compilation.ReplaceSyntaxTree(original, RoslynFixture.Parse(edited, original.FilePath)));
    ...
    public void Pipeline_function_signature_edited_reruns_the_function_output_only()
    {
        ...
        var original = compilation.SyntaxTrees.First();
        var edited = BindingSources.Functions.Replace("public static long Add(long a, long b)",
        ...
        Assert.Contains("global::CESDK.Lua.Marshalling.Int32Marshaller.TryRead(__L, 1, out int __arg0)",
    ...
    public void Pipeline_global_signature_edited_reruns_the_global_output_only()
    {
        ...
        var original = compilation.SyntaxTrees.Last();
        var edited = BindingSources.Globals.Replace("out int value", "out long value", StringComparison.Ordinal);
        var second = GeneratorRun.Execute(first.Driver,
            compilation.ReplaceSyntaxTree(original, RoslynFixture.Parse(edited, original.FilePath)));
    ...
    public void Pipeline_invalid_member_added_keeps_the_table_unchanged_and_the_output_cached()
    {
        ...
        var second = GeneratorRun.Execute(
            ...
            compilation.AddSyntaxTrees(RoslynFixture.Parse(
                "namespace Demo; public static partial class Functions { [CESDK.Annotations.Lua.LuaFunction(\"bad\")] public static int Bad(object o) => 0; }",
                "Bad.cs")));
    ...
    public void Pipeline_second_binding_type_added_emits_its_file_next_to_the_first()
    {
        ...
        var second = GeneratorRun.Execute(
            first.Driver,
            compilation.AddSyntaxTrees(RoslynFixture.Parse(
                "namespace Demo; public static partial class More { [CESDK.Annotations.Lua.LuaFunction(\"more\")] public static int M(int a) => a; }",
                "More.cs")));
    ...
    public void Pipeline_editing_one_of_two_containing_types_leaves_the_others_output_cached()
    {
        ...
        var compilation = roslyn.CreateCompilation(Source);
        var first = RoslynFixture.Run(compilation);
        Assert.Equal(2, first.GeneratedSources.Length);

        var original = compilation.SyntaxTrees.First();
        var edited = Source.Replace("[LuaFunction(\"a\")]", "[LuaFunction(\"aa\")]", StringComparison.Ordinal);
        var second = GeneratorRun.Execute(first.Driver,
            compilation.ReplaceSyntaxTree(original, RoslynFixture.Parse(edited, original.FilePath)));
        ...
        var outputReasons = StepAssert.Reasons(second.Result, LuaBindingsTrackingNames.LuaFunctionOutput);
    ...
    public void Pipeline_step_values_hold_no_roslyn_objects()
    {
        var run = roslyn.Run(
            ...
            "namespace Demo; public static partial class Broken { [CESDK.Annotations.Lua.LuaFunction(\"bad\")] public static int Bad(object o) => 0; [CESDK.Annotations.Lua.LuaGlobal(\"bad\")] public static partial bool TryBad(out object o); }");
