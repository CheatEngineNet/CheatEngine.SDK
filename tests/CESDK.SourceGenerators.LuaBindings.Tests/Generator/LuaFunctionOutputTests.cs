...
public sealed class LuaFunctionOutputTests(RoslynFixture roslyn) : IClassFixture<RoslynFixture>
{
    ...
    {
        var run = roslyn.Run(BindingSources.Functions);
    ...
    {
        var run = roslyn.Run(BindingSources.FunctionSuite);
        ...
        Assert.Contains(
            "global::CESDK.Lua.Marshalling.Utf8Marshaller.TryRead(__L, 1, out global::System.ReadOnlySpan<byte> __arg0)",
            text,
            StringComparison.Ordinal);
        Assert.Contains("global::CESDK.Lua.Marshalling.Utf8Marshaller.Push(__L, __result);", text,
            StringComparison.Ordinal);
        Assert.Contains("global::CESDK.Lua.Marshalling.AddressMarshaller.TryRead(__L, 1, out nuint __arg0)", text,
            StringComparison.Ordinal);
    }
    ...
    public void Generator_registration_is_ordered_by_lua_name()
    {
        ...
        var text = run.SingleGeneratedText;
        var add = text.IndexOf("TrySetGlobal(\"add\"u8)", StringComparison.Ordinal);
        var boom = text.IndexOf("TrySetGlobal(\"boom\"u8)", StringComparison.Ordinal);
        var step = text.IndexOf("TrySetGlobal(\"step\"u8)", StringComparison.Ordinal);
    ...
    {
        var valid = roslyn.Run(BindingSources.Functions);
        var invalid =
