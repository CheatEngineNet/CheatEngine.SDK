...
public sealed class LuaGlobalOutputTests(RoslynFixture roslyn) : IClassFixture<RoslynFixture>
{
    ...
    public void Generator_string_result_reads_through_the_string_marshaller()
    {
        ...
        Assert.Contains("bool __ok = global::CESDK.Lua.Marshalling.StringMarshaller.TryRead(__L, -1, out value);", text,
            StringComparison.Ordinal);
        Assert.Contains("public static partial string ReadString(nuint address, int maxLength)", text,
            StringComparison.Ordinal);
        Assert.Contains(
            "global::CESDK.Lua.CompilerServices.LuaCallSupport.ThrowUnexpectedResult(__L, __top, -1, \"readString\", \"a string\");",
            text, StringComparison.Ordinal);
    }
    ...
    public void Generator_bool_return_without_results_is_the_throwing_form()
    {
        ...
        var body = Section(run.SingleGeneratedText, "public static partial bool IsKeyPressed(int key)",
            "\n        }\n");
        Assert.Contains("global::CESDK.Lua.Marshalling.BooleanMarshaller.TryRead(__L, -1, out bool __result)", body,
            StringComparison.Ordinal);
    ...
    public void Generator_leading_state_parameter_replaces_the_acquisition()
    {
        ...
        var throwing = Section(text,
            "public static partial long AddOn(global::CESDK.Lua.State.LuaState state, long a, long b)",
            "\n        }\n");
        Assert.Contains("global::CESDK.Lua.State.LuaState __L = state;", throwing, StringComparison.Ordinal);
        Assert.DoesNotContain("AcquireState", throwing, StringComparison.Ordinal);
        var tryForm = Section(text,
            "public static partial bool TryAddOn(global::CESDK.Lua.State.LuaState state, long a, long b, out long sum)",
            "\n        }\n");
        Assert.Contains("global::CESDK.Lua.State.LuaState __L = state;", tryForm, StringComparison.Ordinal);
    }
    ...
    public void Generator_nullable_string_annotations_are_repeated_in_the_signature()
    {
        ...
        Assert.Contains("global::CESDK.Lua.Marshalling.Utf8Marshaller.Push(__L, text);", text,
            StringComparison.Ordinal);
    }
