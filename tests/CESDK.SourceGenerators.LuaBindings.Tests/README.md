# CESDK.SourceGenerators.LuaBindings.Tests
...
Generated code must be right for the assemblies it ships with: `CESDK.Annotations`, `CESDK.Lua.Interop` and `CESDK.Lua`.
So this project uses no contract stubs. Every test compiles against the real assemblies. See
the [generator README](../../source-generators/CESDK.SourceGenerators.LuaBindings/README.md).
...
`CESDK.Lua` to the copy the test process runs. So the `LuaRuntime` a test attaches is the one the generated code
...
lookup and the skip reason are described in [`tests/CESDK.Tests.Shared/README.md`](../CESDK.Tests.Shared/README.md).
...
```powershell
dotnet test --project tests/CESDK.SourceGenerators.LuaBindings.Tests
dotnet test --project tests/CESDK.SourceGenerators.LuaBindings.Tests --filter-trait "Category=NativeLua"
```
