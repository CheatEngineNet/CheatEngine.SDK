...
public sealed class LuaGlobalCallEmitterTests
{
        ...
        Assert.Equal(
            """
                internal static void Beep()
                {
                    ...
                    try
                    {
                        if (!global::CESDK.Lua.CompilerServices.LuaGlobalFunctions.TryPush(__L, s_luaGlobal_beep, "beep"u8))
                        {
                            global::CESDK.Lua.CompilerServices.LuaCallSupport.ThrowUnresolvedGlobal(__L, __top, "beep");
                        }

                        global::CESDK.Lua.Calls.LuaStatus __status = __L.TryCall(0, 0);
                        if (!__status.IsOk)
                        {
    ...
    public void Emit_throwing_string_form_reads_a_nullable_local_and_names_the_expected_kind()
    {
        ...
        var text = Emit(model);
        Assert.StartsWith(
            "public static string? ReadString(global::CESDK.Lua.State.LuaState L, nuint address, string? text)\n", text,
            StringComparison.Ordinal);
        Assert.Contains("global::CESDK.Lua.State.LuaState __L = L;\n", text, StringComparison.Ordinal);
        Assert.Contains("global::CESDK.Lua.Marshalling.StringMarshaller.Push(__L, text);\n", text,
            StringComparison.Ordinal);
        Assert.Contains("if (!global::CESDK.Lua.Marshalling.StringMarshaller.TryRead(__L, -1, out string? __result))\n",
            text,
            StringComparison.Ordinal);
    ...
    public void WriteParameterList_writes_state_arguments_and_copy_out_results()
    {
        ...
        Assert.Equal(
            "(global::CESDK.Lua.State.LuaState state, nuint address, int maxLength, global::System.Span<byte> destination, out int written, out string? text)",
            writer.ToString());
