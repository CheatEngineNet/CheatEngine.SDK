...
public sealed class LuaThunkEmitterTests
{
    ...
    public void Emit_void_target_without_arguments_checks_the_count_and_returns_zero()
    {
        Assert.Equal(
            """
                ...
                {
                    global::CESDK.Lua.State.LuaState __L = new(__handle);
                    try
                    {
                        if (__L.Top != 0)
                        {
                            return global::CESDK.Lua.Callbacks.LuaThunk.Fail(__L, "wrong number of arguments to 'ping' (0 expected)"u8);
                        }
                    ...
                    {
                        return global::CESDK.Lua.Callbacks.LuaThunk.Fail(__L, __exception);
                    }
                }
    ...
    public void Emit_state_passing_target_passes_the_state_first_and_pushes_the_result()
    {
        ...
        Assert.Contains("if (!global::CESDK.Lua.Marshalling.DoubleMarshaller.TryRead(__L, 1, out double __arg0))\n",
            text,
            StringComparison.Ordinal);
        Assert.Contains("return global::CESDK.Lua.Callbacks.LuaThunk.FailBadArgument(__L, 1, \"number\"u8);\n", text,
            StringComparison.Ordinal);
        Assert.Contains("bool __result = global::Demo.Suite.IsInteger(__L, __arg0);\n", text, StringComparison.Ordinal);
        Assert.Contains("global::CESDK.Lua.Marshalling.BooleanMarshaller.Push(__L, __result);\n        return 1;\n",
            text,
            StringComparison.Ordinal);
    }
    ...
    public void Registration_registers_and_unregisters_in_the_given_order()
    {
        ...
        Assert.Contains(
            "public static unsafe global::CESDK.Lua.Calls.LuaStatus RegisterLuaFunctions(global::CESDK.Lua.State.LuaState state)\n",
            text, StringComparison.Ordinal);
        Assert.Contains(
            "public static global::CESDK.Lua.Calls.LuaStatus UnregisterLuaFunctions(global::CESDK.Lua.State.LuaState state)\n",
            text, StringComparison.Ordinal);
        Assert.Contains("state.TryPushFunction(new global::CESDK.Lua.Callbacks.LuaNativeFunction(&__LuaThunk_isint));",
            text,
            StringComparison.Ordinal);
        Assert.True(
    ...
    public void Registration_puts_the_member_attributes_on_both_methods()
    {
        ...
        Assert.Contains("[Marker]\npublic static unsafe global::CESDK.Lua.Calls.LuaStatus RegisterLuaFunctions", text,
            StringComparison.Ordinal);
        Assert.Contains("[Marker]\npublic static global::CESDK.Lua.Calls.LuaStatus UnregisterLuaFunctions", text,
            StringComparison.Ordinal);
    }
