...
`CESDK.Tests.Shared` is a project that runs no test. Its `NativeLua` fixture binds `CESDK.Lua.Interop.Api.LuaApi` to a
Lua 5.3 DLL
once per process and hands out independent Lua states.

## Why it exists

Tests tagged `Category=NativeLua` and the benchmarks need real Lua behavior that no fake reproduces. The fixture exists
once, in one project with one namespace, `CESDK.Tests.Shared.NativeLua`. Linked source gave the same file several owners
and a namespace that followed whichever project a tool evaluated last. The cost is one more node in the reference graph
and an `InternalsVisibleTo` list in the project file.
...
Reference the project with `<ProjectReference Include="../CESDK.Tests.Shared/CESDK.Tests.Shared.csproj"/>` and add the
referencing project to the `InternalsVisibleTo` list of `CESDK.Tests.Shared.csproj`, because the types are internal.
The referencing project needs `AllowUnsafeBlocks` and implicit usings (on for everything under `tests/`);
`CESDK.Lua.Interop` flows through this project. A new file in the `NativeLua` folder needs no project edit.
...
The projects that use `NativeLua` are `CESDK.Lua.Interop.Tests`, `CESDK.Lua.Tests`, `CESDK.Engine.Tests`,
`CESDK.Hosting.Tests`, `CESDK.SourceGenerators.EngineApi.Tests`, `CESDK.SourceGenerators.LuaBindings.Tests` and
`CESDK.Benchmarks`. Benchmarks call `NativeLuaLibrary.ThrowIfUnavailable()` in `[GlobalSetup]` instead of skipping.
...
- Every type is `internal` and free of xUnit types. `CESDK.Benchmarks` uses it without an xUnit reference.
