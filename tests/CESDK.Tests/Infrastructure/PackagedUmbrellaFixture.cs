...
/// <summary>
///     Packs <c>src/CESDK/CESDK.csproj</c> once, to a throwaway local feed, and restores + builds three throwaway
///     consumer plugin projects against it: a default one that takes every package default, one that sets
///     <c>AllowUnsafeBlocks=false</c> itself, and one that sets <c>CesdkGenerateEntryPoint=false</c>. Every fact
...
public sealed class PackagedUmbrellaFixture : IAsyncLifetime
{
    ...
    /// <summary>
    ///     Whether <c>CESDK.CESDK</c> exists for the consumer that set <c>CesdkGenerateEntryPoint=false</c> (it must
    ///     not).
    /// </summary>
    ...
    {
        _tempRoot = Directory.CreateTempSubdirectory("cesdk-umbrella-tests-");
        ...
        DefaultProperties = await defaultConsumer.GetPropertiesAsync(BuildTimeout, "AllowUnsafeBlocks",
                "EnableDynamicLoading", "CesdkGenerateEntryPoint")
        ...
        var entryPointOffConsumer = ThrowawayConsumer.Create(
            ...
            "    <CesdkGenerateEntryPoint>false</CesdkGenerateEntryPoint>\n");
    ...
    private static async Task PackUmbrellaAsync(string feedDirectory)
    {
        ...
        var cesdkProjectPath = RepositoryLayout.PathOf("src/CESDK/CESDK.csproj");
        ProcessResult result = default;
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            result = await ProcessRunner
                .RunAsync("dotnet", $"pack \"{cesdkProjectPath}\" -c Release -o \"{feedDirectory}\" --nologo",
    ...
    {
        var nupkgPaths = Directory.GetFiles(feedDirectory, "CESDK.*.nupkg");
        if (nupkgPaths.Length != 1)
            throw new InvalidOperationException(
                $"Expected exactly one CESDK.*.nupkg in '{feedDirectory}', found {nupkgPaths.Length}: {string.Join(", ", nupkgPaths)}");

        var fileName = Path.GetFileName(nupkgPaths[0]);
        PackageVersion = fileName["CESDK.".Length..^".nupkg".Length];
