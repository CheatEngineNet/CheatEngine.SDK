...
/// <summary>
///     The packed umbrella must not carry a <c>CESDK.*</c> nuspec dependency for any of the six embedded libs.
///     <c>PrivateAssets="all"</c> on every embedded <c>ProjectReference</c> (<c>src/CESDK/CESDK.csproj</c>) is what
...
{
    [Fact]
    public void Packed_nuspec_declares_no_CESDK_dependency()
    {
        Assert.DoesNotContain(fixture.NuspecDependencyIds,
            id => id.StartsWith("CESDK", StringComparison.OrdinalIgnoreCase));
    }
    ...
    public void Packed_nuspec_declares_no_dependency_at_all()
    {
        ...
        // an empty dependency group, not merely one without a CESDK.* entry.
