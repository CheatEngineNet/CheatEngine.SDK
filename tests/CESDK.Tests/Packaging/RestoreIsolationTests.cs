...
///     extraction of <c>cesdk/&lt;version&gt;</c> already sat there from an earlier run - this fixture's own previous
...
public sealed class RestoreIsolationTests(PackagedUmbrellaFixture fixture)
{
    ...
    public void Fixture_exposes_a_packages_directory_under_its_own_temp_root()
    {
        ...
        Assert.Contains("cesdk-umbrella-tests-", fixture.PackagesDirectory, StringComparison.Ordinal);
    }
    ...
    public void Consumer_restore_extracts_the_packed_version_into_the_isolated_packages_directory()
    {
        ...
        // '<packagesDirectory>/cesdk/<version>' as a side effect of extracting the package there. If a future
        // change dropped '--packages "<packagesDirectory>"' from ThrowawayConsumer.RestoreAsync, this directory
        // would stay empty while restore quietly fell back to the machine-wide global-packages folder instead.
        var extractedPackageDirectory = Path.Combine(fixture.PackagesDirectory, "cesdk", fixture.PackageVersion);
        Assert.True(
            Directory.Exists(extractedPackageDirectory),
            $"Expected the consumer restore to extract 'cesdk/{fixture.PackageVersion}' into the fixture's isolated " +
            $"packages directory ('{extractedPackageDirectory}'), not the machine-wide global-packages folder.");
    }
}
