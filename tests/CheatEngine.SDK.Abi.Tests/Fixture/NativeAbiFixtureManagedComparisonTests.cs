...
public sealed unsafe class NativeAbiFixtureManagedComparisonTests
{
	...
	public void Native_fixture_layout_facts_match_the_managed_x64_measurements_when_CI_supplies_them()
	{
		...
		Dictionary<string, string> nativeFacts = ReadFacts(factsPath);
		Dictionary<string, string> managedFacts = CreateManagedLayoutFacts();
	...
	public void Native_fixture_required_mode_rejects_an_absent_facts_path(string? factsPath)
	{
		InvalidOperationException exception =
			Assert.Throws<InvalidOperationException>(() => ResolveFactsPath(factsPath, "true"));
	...
	public void Native_fixture_comparison_rejects_a_supplied_missing_facts_file()
	{
		...
		FileNotFoundException exception = Assert.Throws<FileNotFoundException>(() => ReadFacts(factsPath));
	...
	private static Dictionary<string, string> CreateManagedLayoutFacts()
	{
		...
		AddOffset(facts, "plugin_type0_record.Address",
			Layout.OffsetOf(pluginType0RecordOrigin, &pluginType0Record.Address));
		AddOffset(facts, "plugin_type0_record.IsPointer",
			Layout.OffsetOf(pluginType0RecordOrigin, &pluginType0Record.IsPointer));
		AddOffset(facts, "plugin_type0_record.CountOffsets",
			Layout.OffsetOf(pluginType0RecordOrigin, &pluginType0Record.CountOffsets));
		AddOffset(facts, "plugin_type0_record.Offsets",
			Layout.OffsetOf(pluginType0RecordOrigin, &pluginType0Record.Offsets));
		AddOffset(facts, "plugin_type0_record.Description",
			Layout.OffsetOf(pluginType0RecordOrigin, &pluginType0Record.Description));
		AddOffset(facts, "plugin_type0_record.ValueType",
			Layout.OffsetOf(pluginType0RecordOrigin, &pluginType0Record.ValueType));
	...
	private static void AddExportedFunctionsPrefixOffsets(Dictionary<string, string> facts)
	{
		...
		AddOffset(facts, "exported_functions_prefix.RegisterFunction",
			Layout.OffsetOf(origin, &exports.RegisterFunction));
		AddOffset(facts, "exported_functions_prefix.UnregisterFunction",
			Layout.OffsetOf(origin, &exports.UnregisterFunction));
		AddOffset(facts, "exported_functions_prefix.OpenedProcessId",
			Layout.OffsetOf(origin, &exports.OpenedProcessId));
