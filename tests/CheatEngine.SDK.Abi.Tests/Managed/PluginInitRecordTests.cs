...
public sealed unsafe class PluginInitRecordTests
{
	...
	public void Write_through_a_pointer_on_64_bit_touches_exactly_36_bytes(int start)
	{
		...
		const int Total = 80;
		byte* buffer = stackalloc byte[Total];
		Span<byte> bytes = new(buffer, Total);
		bytes.Fill(Guard);

		PluginInitRecord* record = (PluginInitRecord*) (buffer + start);
		*record = default;
	...
	public void Callbacks_stored_in_the_record_are_callable_with_the_declared_shapes()
	{
		...
		Bool32 versionResult = record.GetVersion(&version, Layout.SizeOf<PluginVersion>());
	...
	{
		if (version is null || size < sizeof(PluginVersion))
		{
			return Bool32.False;
		}
