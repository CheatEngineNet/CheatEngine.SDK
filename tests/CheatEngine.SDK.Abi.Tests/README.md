...
| Native-fact comparison | The native CI job provides the checked `ce77-native-abi-facts.txt`; a compiled managed test measures every fixture-covered layout and compares its size, alignment, and offsets to that output. |
...
`NativeAbiFixtureManagedComparisonTests` has no local fixture dependency: its comparison is activated only by the
native CI job through `CE77_NATIVE_ABI_FACTS_PATH`. The absence of that CI-only input is intentional and is not a
substitute for an exact-host test.
