...
public sealed unsafe class AbiShapeTests
{
	...
	{
		string? violation = AbiShape.FindViolation(structure, Trusted);
	...
	{
		string? violation = AbiShape.FindViolation(typeof(ExplicitLayout), Trusted);
	...
	public void FindViolation_structure_of_another_assembly_is_foreign()
	{
		...
		string? violation = AbiShape.FindViolation(typeof(Conforming), typeof(PluginInitRecord).Assembly);
