...
public sealed class DiagnosticCatalogTests
{
	...
	{
		DiagnosticDescriptor descriptor = Descriptor(id);
		string title = descriptor.Title.ToString(CultureInfo.InvariantCulture);
		string description = descriptor.Description.ToString(CultureInfo.InvariantCulture);
	...
	public void Descriptor_has_a_documentation_page_and_a_release_tracking_row(string id)
	{
		...
		string page = RepositoryLayout.PathOf($"analyzers/docs/{id}.md");
	...
	{
		foreach (string line in tracking.Split('\n'))
		{
			string[] cells = line.Split('|');
			if (cells.Length >= 3
	...
	public void Every_plugin_class_problem_has_its_own_message_and_a_place_in_the_report_order()
	{
		...
		string fallback = PluginClassProblemText.Describe(PluginShapeIssues.None);
