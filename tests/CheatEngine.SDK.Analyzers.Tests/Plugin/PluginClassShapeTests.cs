...
public sealed class PluginClassShapeTests
{
	...
	public async Task Class_that_takes_the_name_of_the_generated_entry_point_reports_reserved_name(string declaration,
		string displayName)
	{
		bool directCollision = string.Equals(displayName, "CESDK", StringComparison.Ordinal);
		DiagnosticResult collision = directCollision
			...
			: Verifier.Diagnostic(DiagnosticDescriptors.GeneratedEntryPointCollision)
				...
				.WithArguments("CESDK");
		DiagnosticResult reservedName = Problem(0, displayName, PluginShapeIssues.ReservedEntryPointName);
