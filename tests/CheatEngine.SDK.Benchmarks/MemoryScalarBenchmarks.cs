...
public class MemoryScalarBenchmarks : IDisposable
{
	...
	public void Setup()
	{
		...
		LuaState state = FakeHostRuntime.Attach(_state, false);
		LuaStatus defined = state.TryExecute(ScalarStandIns, 0);
		if (!defined.IsOk)
		{
	...
	{
		if (!TargetMemory.TryReadInt32(Address32, out int value, out _))
		{
	...
	{
		if (!TargetMemory.TryReadInt64(Address64, out long value, out _))
		{
	...
	{
		if (!HostMemory.TryReadInt32(HostAddress32, out int value, out _))
		{
	...
	{
		if (!HostMemory.TryReadInt64(HostAddress64, out long value, out _))
		{
