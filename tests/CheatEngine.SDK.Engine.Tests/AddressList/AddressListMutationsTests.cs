...
public sealed class AddressListMutationsTests
{
	...
	public void Delete_reports_each_address_list_preflight_failure_without_starting_a_mutation()
	{
		...
		MemoryRecordMutationOutcome missingGlobal = AddressListMutations.Delete(new MemoryRecordId(1));
	...
	public void Delete_distinguishes_missing_record_from_success_and_restores_stack()
	{
		...
		int top = fixture.State.Top;

		MemoryRecordMutationOutcome missing = AddressListMutations.Delete(new MemoryRecordId(99));
		...
		MemoryRecordMutationOutcome success = AddressListMutations.Delete(new MemoryRecordId(1));
	...
	public void Delete_preserves_a_non_callable_record_lookup_failure()
	{
		...
		MemoryRecordMutationOutcome result = AddressListMutations.Delete(new MemoryRecordId(1));
	...
	public void Set_parent_rejects_self_missing_and_cyclic_hierarchies_before_assignment()
	{
		...
		MemoryRecordMutationOutcome self = AddressListMutations.SetParent(new MemoryRecordId(1), new MemoryRecordId(1));
		MemoryRecordMutationOutcome missing =
			AddressListMutations.SetParent(new MemoryRecordId(1), new MemoryRecordId(99));
		fixture.Execute("parent.Parent = child");
		MemoryRecordMutationOutcome
			cycle = AddressListMutations.SetParent(new MemoryRecordId(1), new MemoryRecordId(2));
	...
	public void Set_parent_obeys_an_explicit_parent_walk_bound()
	{
		...
		MemoryRecordMutationOutcome result = AddressListMutations.SetParent(new MemoryRecordId(1),
			new MemoryRecordId(2),
	...
	public void Set_parent_assigns_a_typed_parent_or_lua_nil_for_root()
	{
		...
		MemoryRecordMutationOutcome parent =
			AddressListMutations.SetParent(new MemoryRecordId(1), new MemoryRecordId(2));
		fixture.Execute("assert(assigned == 1)");
		MemoryRecordMutationOutcome root = AddressListMutations.SetParent(new MemoryRecordId(1), null);
	...
	public void Started_destroy_or_parent_assignment_error_is_indeterminate_and_not_retried()
	{
		...
		MemoryRecordMutationOutcome destroy = AddressListMutations.Delete(new MemoryRecordId(1));
		...
		using MutationFixture failingSetter = new(true);
		MemoryRecordMutationOutcome assignment =
			AddressListMutations.SetParent(new MemoryRecordId(1), new MemoryRecordId(2));
	...
	private sealed class MutationFixture : IDisposable
	{
		...
		public MutationFixture(bool parentSetterFails = false, bool destroyFails = false)
		{
			...
			State = _scope.State;
			CEObject listObject = FakeHost.CreateObject(State, "Probe");
			string childInitializer = parentSetterFails
			...
			CEObject childObject = FakeHost.CreateObject(State, destroyFails ? "Stubborn" : "Probe", childInitializer);
			CEObject parentObject = FakeHost.CreateObject(State, "Probe", "o.props.ID = 2");
			CEObject grandparentObject = FakeHost.CreateObject(State, "Probe", "o.props.ID = 3");
			...
			where T : struct, ILuaMarshaller<T>
		{
			...
			if (!status.IsOk)
			{
				Assert.Fail("Setting a test global failed: " + EngineTest.ErrorMessage(state, status));
			}
		}
	}
}
