...
public sealed class TargetMemoryOutcomeTests
{
    ...
    {
        var outcome = TargetMemoryOperationOutcome.Succeeded();
    ...
    [InlineData(EngineFailureKind.TargetIdentityUnavailable, TargetMemoryOperationOutcomeKind.TargetIdentityUnavailable)]
    ...
    {
        var outcome = TargetMemoryOperationOutcome.Failed(failureKind, LuaStatus.RuntimeError);
    ...
    {
        var outcome = TargetMemoryOperationOutcome.Failed(EngineFailureKind.ProtectedLuaFailure,
    ...
    {
        var outcome = TargetMemoryAllocationOutcome.Succeeded(new Address(0x7FF6_4000_0000));
    ...
    {
        var failure = TargetMemoryOperationOutcome.Failed(EngineFailureKind.ExpectedOperationFailure);
        var outcome = TargetMemoryAllocationOutcome.Failed(failure);
