...
public sealed class CEEnumNamesTests
{
	...
	public void Name_lookup_allocates_nothing()
	{
		...
		AllocationGate.AssertZero(() =>
		{
			...
			if (CEEnumNames.TryParseCEName("soExactValue"u8, out ScanOption option))
			{
				sink += (int) option;
			}
		});
		...
		where TEnum : struct, Enum
	{
		...
		{
			ReadOnlySpan<byte> name = nameOf(member);
			Assert.False(name.IsEmpty, member + " has no CE name.");
			Assert.True(parse(name, out TEnum parsed), member + " does not parse back.");
			Assert.Equal(member, parsed);
		}
	}
