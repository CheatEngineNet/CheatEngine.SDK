...
public sealed class EngineInspectionTests
{
	...
	public void EnumerateModules_current_and_explicit_process_copy_ce77_fields_and_preserve_stack()
	{
		...
		InspectionStatus status = EngineInspection.EnumerateModules(current, out int currentCount);
		...
		status = EngineInspection.EnumerateModules(new TargetProcessId(4242), explicitProcess, out int explicitCount);
	...
	public void EnumerateModules_destination_too_small_writes_nothing()
	{
		...
		int top = L.Top;
		InspectionStatus status = EngineInspection.EnumerateModules(destination, out int written);
	...
	public void EnumerateSections_address_and_name_selectors_copy_file_offsets_without_confusing_them_with_addresses()
	{
		...
		int top = L.Top;
		InspectionStatus status = EngineInspection.EnumerateSections(0x140000000UL, sections, out int written);
	...
	public void ResolveAddress_distinguishes_a_nil_miss_from_a_zero_address_and_lua_failure()
	{
		...
		InspectionStatus status =
			EngineInspection.ResolveAddress(new SymbolExpression("zero"), default, out Address zero);
		...
		status = EngineInspection.ResolveAddress(new SymbolExpression("missing"), default, out Address missing);

		Assert.Equal(InspectionStatus.NotFound, status);
		Assert.Equal(Address.Zero, missing);
		Assert.Equal(top, L.Top);

		status = EngineInspection.ResolveAddress(new SymbolExpression("broken"), default, out Address failed);
	...
	public void ResolveAddress_reports_an_unavailable_global_without_entering_lua()
	{
		...
		InspectionStatus status =
			EngineInspection.ResolveAddress(new SymbolExpression("missingGlobal"), default, out Address address);
	...
	public void Inspection_preserves_lua_failures_from_global_and_table_field_resolution()
	{
		...
		int top = L.Top;
		InspectionStatus status =
			EngineInspection.ResolveAddress(new SymbolExpression("any"), default, out Address address);
		Assert.Equal(InspectionStatus.LuaFailure, status);
		Assert.Equal(Address.Zero, address);
		Assert.Equal(top, L.Top);

		status = EngineInspection.GetSymbolInfo(new SymbolExpression("any"), out SymbolInfo symbol);
		Assert.Equal(InspectionStatus.LuaFailure, status);
	...
	public void GetSymbolInfo_uses_the_symbol_list_canonical_symbolsize_field_and_preserves_nil_absence()
	{
		...
		int top = L.Top;
		InspectionStatus status =
			EngineInspection.GetSymbolInfo(new SymbolExpression("Game.Update"), out SymbolInfo symbol);
	...
	public void Memory_regions_copy_windows_fields_optional_extra_and_reject_malformed_or_nil_single_records()
	{
		...
		int top = L.Top;
		InspectionStatus status = EngineInspection.EnumerateMemoryRegions(regions, out int written);
		...
		status = EngineInspection.GetMemoryRegionInfo(0x140000000UL, out MemoryRegionInfo one);
		Assert.Equal(InspectionStatus.Success, status);
