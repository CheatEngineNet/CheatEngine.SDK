...
public sealed class MemoryApiTests
{
	...
	public void Target_scalars_preserve_signedness_widths_pointer_bits_and_floating_point_values()
	{
		...
		Assert.True(TargetMemory.TryReadUInt8(16UL, out byte u8, out MemoryAccessFailure failure));
	...
	public void Target_buffers_and_strings_keep_their_order_and_never_return_a_dangling_lua_span()
	{
		...
		Assert.True(TargetMemory.TryReadBytes(32UL, bytes, out MemoryAccessFailure failure));
		Assert.Equal(MemoryAccessFailure.None, failure);
	...
	public void Target_pointer_width_qualification_uses_the_observed_target_not_the_host_and_refuses_x86_overflow()
	{
		...
		Assert.True(TargetMemory.TryReadPointer(23UL, PointerSize.Bit32, out Address x86Pointer,
			out MemoryAccessFailure failure));
		Assert.Equal(0xFEDCBA98UL, x86Pointer.Value);
		Assert.True(TargetMemory.TryReadPointer(24UL, PointerSize.Bit64, out Address x64Pointer, out failure));
		Assert.Equal(0x123456789ABCDEFUL, x64Pointer.Value);
		Assert.False(TargetMemory.TryReadPointer(25UL, PointerSize.Bit32, out Address overflowPointer, out failure));
		Assert.Equal(MemoryAccessFailure.PointerValueExceedsTargetWidth, failure);
		Assert.Equal(0UL, overflowPointer.Value);
		Assert.False(TargetMemory.TryReadPointer(23UL, PointerSize.Unknown, out Address unknownWidthPointer,
			out failure));
	...
	[Fact]
	public void
		Detailed_target_buffer_reads_preserve_a_partial_prefix_and_required_utf8_capacity_without_lua_owned_storage()
	{
		EngineTest.RequireNativeLua();
		using NativeLuaState state = new();
		using HostScope scope = new(state);
		...
		Assert.False(TargetMemory.TryReadBytes(37UL, detailedDestination, out int copied,
			out MemoryAccessFailure failure));
		...
		tooSmall[0] = 0xA5;
		Assert.False(TargetMemory.TryReadUtf8(51UL, 100, tooSmall, false, out int written,
			out int requiredLength, out failure));
		...
		Assert.True(TargetMemory.TryReadUtf8(51UL, 100, exact, false, out written,
	...
	public void Detailed_host_buffer_read_preserves_a_partial_prefix()
	{
		...
		Assert.False(HostMemory.TryReadBytes(new HostAddress(84), destination, out int copied,
			out MemoryAccessFailure failure));
		Assert.Equal(MemoryAccessFailure.PartialRead, failure);
		Assert.Equal(1, copied);
	...
	public void Byte_writes_require_the_full_CE_count_for_target_and_host_memory()
	{
		...
		Assert.True(TargetMemory.TryWriteBytes(33UL, payload, out MemoryAccessFailure failure));
		Assert.Equal(MemoryAccessFailure.None, failure);
		Assert.False(TargetMemory.TryWriteBytes(34UL, payload, out int targetPartialCount, out failure));
		Assert.Equal(MemoryAccessFailure.WriteFailed, failure);
		Assert.Equal(payload.Length - 1, targetPartialCount);
		Assert.False(TargetMemory.TryWriteBytes(35UL, payload, out int targetZeroCount, out failure));
		Assert.Equal(MemoryAccessFailure.WriteFailed, failure);
		Assert.Equal(0, targetZeroCount);
		Assert.False(TargetMemory.TryWriteBytes(38UL, payload, out int targetInvalidCount, out failure));
		Assert.Equal(MemoryAccessFailure.InvalidResult, failure);
		...
		Assert.Equal(MemoryAccessFailure.None, failure);
		Assert.False(HostMemory.TryWriteBytes(new HostAddress(82), payload, out int hostPartialCount, out failure));
		Assert.Equal(MemoryAccessFailure.WriteFailed, failure);
		Assert.Equal(payload.Length - 1, hostPartialCount);
		Assert.False(HostMemory.TryWriteBytes(new HostAddress(83), payload, out int hostZeroCount, out failure));
		Assert.Equal(MemoryAccessFailure.WriteFailed, failure);
	...
	public void Host_scalars_use_host_addresses_and_the_documented_local_byte_table_for_8_bit_access()
	{
		...
		Assert.True(HostMemory.TryReadUInt8(byteAddress, out byte u8, out MemoryAccessFailure failure));
		Assert.Equal(byte.MaxValue, u8);
	...
	public void Failures_are_classified_without_leaking_stack_values()
	{
		...
		Assert.False(TargetMemory.TryReadUInt32(999UL, out _, out MemoryAccessFailure failure));
		Assert.Equal(MemoryAccessFailure.ReadFailed, failure);
	...
	public void Scalar_number_reads_reject_coercible_Lua_strings()
	{
		...
		Assert.False(TargetMemory.TryReadDouble(28UL, out _, out MemoryAccessFailure failure));
		Assert.Equal(MemoryAccessFailure.InvalidResult, failure);
		Assert.Equal(0, scope.State.Top);
	}
	...
	public void A_missing_global_is_distinct_from_a_detached_runtime()
	{
		...
		using (HostScope scope = new(state))
		{
			Assert.False(TargetMemory.TryReadPointer(1UL, out _, out MemoryAccessFailure failure));
