...
public sealed class StringListTests
{
	...
	public void Handle_properties_and_zero_based_indexer_round_trip_without_stack_residue()
	{
		...
		Assert.True(list.TryGetCount(out int count));
		Assert.Equal(2, count);
		Assert.True(list.TryGetItem(0, out string? first));
		Assert.Equal("00401000", first);
		Assert.True(list.TrySetItem(1, "00402000"));
		Assert.True(list.TryGetItem(1, out string? replacement));
		Assert.Equal("00402000", replacement);

		Assert.True(list.TryGetSorted(out bool sorted));
		Assert.False(sorted);
		Assert.True(list.TrySetSorted(true));
		Assert.True(list.TryGetSorted(out sorted));
		Assert.True(sorted);
		Assert.True(list.TryGetCaseSensitive(out bool caseSensitive));
	...
	public void Strings_methods_preserve_CE_zero_based_indices_and_update_the_list()
	{
		...
		Assert.True(list.TryAdd("00403000", out int addedIndex));
		Assert.Equal(2, addedIndex);
		Assert.True(list.TryIndexOf("00403000", out int foundIndex));
		Assert.Equal(2, foundIndex);
		Assert.True(list.TryIndexOf("missing", out int missingIndex));
		Assert.Equal(-1, missingIndex);
		Assert.True(list.TryGetText(out string? text));
		...
		Assert.True(list.TryGetItem(0, out string? onlyLine));
	...
	public void Duplicates_property_round_trips_each_numeric_CE_enum_value(DuplicateHandling expected)
	{
		...
		Assert.True(list.TryGetDuplicates(out DuplicateHandling duplicates));
	...
	public void Factory_returns_an_owned_list_that_destroys_deterministically()
	{
		...
		Assert.True(StringLists.TryCreate(out Owned<StringList>? created));
