...
internal static unsafe partial class FakeHost // NOSONAR: the fixture implements Cheat Engine's unmanaged callback ABI.
{
	...
	public static CEObject CreateObject(LuaState L, string className, string initializer = "")
	{
		...
		LuaStatus status = L.TryLoad(utf8, "=fakehost"u8);
	...
	{
		using LuaFrame frame = new(L);
		LuaStatus status = L.TryExecute(Model, 3, "=fakehost"u8);
	...
	private static void FailIfNotOk(LuaState L, LuaStatus status, string what)
	{
		if (!status.IsOk)
		{
			Assert.Fail("The fake host failed while " + what + ": " + LuaError.FromStack(L, status));
		}
	}
	...
	private static void PushObject(void* L, void* nativeObject)
	{
		...
		void* block = lua_newuserdata(state, (nuint) sizeof(nint));
