...
public sealed class CheatTableFilesTests
{
    ...
    public void TryLoad_and_try_save_forward_opaque_path_and_merge_arguments()
    {
        ...
        var top = L.Top;
        var load = CheatTableFiles.TryLoad("../unrestricted.ct", merge: true);
        ...
        var save = CheatTableFiles.TrySave("C:/profiles/current.ct");
    ...
    public void Table_file_calls_distinguish_missing_globals_and_lua_failures()
    {
        ...
        Assert.Equal(LuaOperationStatusKind.GlobalUnavailable, CheatTableFiles.TryLoad("missing.ct", merge: false).Kind);
        ...
        Assert.Equal(LuaOperationStatusKind.LuaFailure, CheatTableFiles.TryLoad("broken.ct", merge: false).Kind);
        Assert.Equal(top, L.Top);
