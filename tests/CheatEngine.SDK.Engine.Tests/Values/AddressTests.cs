...
public sealed class AddressTests
{
	...
	{
		Assert.True(Address.TryParse(text, out Address fromChars));
		Assert.Equal(expected, fromChars.Value);

		Assert.True(Address.TryParse(Encoding.UTF8.GetBytes(text), out Address fromBytes));
	...
	{
		Assert.False(Address.TryParse(text, out Address fromChars));
		Assert.Equal(Address.Zero, fromChars);

		Assert.False(Address.TryParse(Encoding.UTF8.GetBytes(text), out Address fromBytes));
	...
	{
		Assert.False(Address.TryParse((string?) null, out Address address));
...
#pragma warning restore MA0011
	...
	public void TryFormat_writes_utf16_and_utf8_and_reports_a_small_buffer()
	{
		...
		Assert.True(address.TryFormat(chars, out int charsWritten, default, null));
		...
		Assert.True(address.TryFormat(bytes, out int bytesWritten, default, null));
		...
		Assert.True(Address.TryParse(bytes[..bytesWritten], out Address roundTripped));
	...
	public void Parsing_and_formatting_do_not_depend_on_the_current_culture(string cultureName)
	{
		...
		try
		{
			...
			Address address = Address.Parse("0x7ff6a1b2c3d4");
...
#pragma warning restore MA0011
			...
			Assert.True(Address.TryParse("00000000000000FF", out Address parsed));
	...
	{
		Address high = Address.FromInt64(-1);
		...
		Address low = Address.FromUInt64(0x400000);
