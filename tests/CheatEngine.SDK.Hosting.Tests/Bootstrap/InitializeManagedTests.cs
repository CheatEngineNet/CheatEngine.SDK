...
public sealed unsafe class InitializeManagedTests
{
	...
	public void Writes_exactly_the_36_byte_record_and_nothing_past_it()
	{
		...
		int result = host.Initialize<RecordingPluginFactory>();
		...
		ref PluginInitRecord record = ref host.Record;
	...
	public void Name_is_the_ASCII_bytes_of_the_factory_name_NUL_terminated()
	{
		...
		ReadOnlySpan<byte> written = AnsiNameBuffer.Read(host.Record.Name);
	...
	public void Non_ASCII_name_is_converted_to_the_process_ANSI_code_page()
	{
		...
		IntPtr reference = Marshal.StringToHGlobalAnsi(NonAsciiNamePluginFactory.Name);
		try
		{
			ReadOnlySpan<byte> expected = MemoryMarshal.CreateReadOnlySpanFromNullTerminated((byte*) reference);
			ReadOnlySpan<byte> written = AnsiNameBuffer.Read(host.Record.Name);
	...
	public void Null_record_address_fails_and_is_logged()
	{
		...
		int result = PluginHost.InitializeManaged<RecordingPluginFactory>(0, 36);
	...
	public void A_factory_whose_name_getter_throws_fails_with_0_and_nothing_registered_so_a_good_factory_still_can()
	{
		...
		int result = broken.Initialize<ThrowingNamePluginFactory>();
		...
		NotSupportedException exception = Assert.IsType<NotSupportedException>(entry.Item3);
