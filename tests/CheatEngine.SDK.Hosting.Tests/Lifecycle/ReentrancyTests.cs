...
public sealed unsafe class ReentrancyTests
{
	...
	public void A_concurrent_disable_during_OnEnable_fails_immediately_and_the_outer_enable_decides_the_state()
	{
		...
		Exception? earlyDispatch = Record.Exception(() => MainThread.Invoke(static _ =>
		{
		}, 0));
		InvalidOperationException earlyDispatchFailure = Assert.IsType<InvalidOperationException>(earlyDispatch);
	...
	public void Disable_nested_in_OnEnable_is_refused_and_the_enable_stands()
	{
		...
		Bool32 outer = host.CallEnable(&exports, 1);
	...
	{
		int count = 0;
		foreach ((HostLogLevel entryLevel, string message, Exception? _) in sink.Entries)
		{
