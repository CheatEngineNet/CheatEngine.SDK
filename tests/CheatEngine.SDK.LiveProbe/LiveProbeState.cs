...
internal static unsafe class LiveProbeState
{
    ...
    {
        if (bootstrapResult != ManagedEntryPoint.Success || initRecord == 0) return;
        ...
        try
        {
            var tail = (byte*)initRecord + sizeof(PluginInitRecord);
            var before = ReadUnalignedUInt32(tail);
    ...
    internal static string GetStatus()
    {
        lock (Gate)
        {
            ...
            builder.Append("CE 7.7 live probe status; bootstrapCalls=").Append(s_bootstrap.Calls)
                .Append(", bootstrapAddress=0x").Append(s_bootstrap.InitRecord.ToString("X", CultureInfo.InvariantCulture))
            ...
            if (s_bootstrap.TailCanaryWritten)
                builder.Append(", tailBefore=0x").Append(s_bootstrap.TailReadBeforeWrite.ToString("X8", CultureInfo.InvariantCulture))
                    .Append(", expectedPreviousCanary=0x").Append(TailCanary.ToString("X8", CultureInfo.InvariantCulture));

            if (!string.IsNullOrEmpty(s_bootstrap.TailFailure)) builder.Append(", tailFailure=").Append(s_bootstrap.TailFailure);
    ...
    internal static string BeginSynchronizeProbe()
    {
        ...
        lock (Gate)
        {
            if (s_synchronize.IsRunning) return "Synchronize probe is already running; call ce77_live_probe_synchronize_status().";

            s_synchronize = SynchronizeObservation.Started;
        }

        var thread = new Thread(RunSynchronizeProbe)
        {
            IsBackground = true,
    ...
    internal static string BeginLuaThreadProbe()
    {
        ...
        var serial = Interlocked.Increment(ref s_luaProbeSerial);
        var token = string.Create(CultureInfo.InvariantCulture, $"ce77-live-probe-registry-{serial}");
        using (var frame = new LuaFrame(state))
        {
        ...
        lock (Gate)
        {
            if (s_luaThread.IsRunning) return "Lua thread probe is already running; call ce77_live_probe_lua_threads_status().";
        ...
        var thread = new Thread(RunLuaThreadProbe)
        {
            IsBackground = true,
            Name = "CheatEngine.SDK CE77 Lua thread probe"
        };
        thread.Start();
        return "Lua thread/registry probe started. Do not run other Lua code for one second; poll ce77_live_probe_lua_threads_status().";
    }
    ...
    internal static string SnapshotBeforeReset()
    {
        ...
        return "Reset snapshot captured. In CE's Lua Engine call resetLuaState() manually, then call ce77_live_probe_snapshot_after_reset(). The SDK does not support an external reset that it was not told about; this only records the raw outcome.";
    }
    ...
    internal static string SnapshotAfterReset()
    {
        ...
        if (reference is null) return "No before-reset snapshot exists. Call ce77_live_probe_snapshot_before_reset() first.";
        ...
        using (var frame = new LuaFrame(state))
        {
            ...
            if (oldReferencePushed) _ = state.TryReadString(-1, out oldReferenceValue);
        }
    ...
    internal static string ObserveHostUserdata()
    {
        ...
        var state = LuaRuntime.AcquireState();
        using var frame = new LuaFrame(state);
        var status = state.TryExecute("local value = getMainForm(); return type(value), tostring(value)"u8, 2,
            "=ce77_live_probe_userdata"u8);
        if (!status.IsOk) return "getMainForm userdata observation failed with " + status + ".";

        var hasType = state.TryReadString(-2, out var type);
        var hasText = state.TryReadString(-1, out var text);
    ...
    internal static string PrepareCallbackShutdownProbe()
    {
        ...
        lock (Gate)
        {
            if (s_callback is { IsReleased: false })
                return "Callback shutdown probe is already prepared. Disable this plugin, then run pcall(ce77_live_probe_callback_shutdown) from CE's Lua Engine.";
        }
        ...
        using (var frame = new LuaFrame(state))
        {
            var createStatus = LuaCallback.TryCreate(state, new LuaNativeFunction(&CallbackShutdownThunk), counter, out callback);
            if (!createStatus.IsOk || callback is null)
                return "Callback shutdown probe could not create its callback: " + createStatus + ".";

            var registerStatus = callback.TryRegister(state, "ce77_live_probe_callback_shutdown"u8);
        ...
        return "Callback prepared. First run pcall(ce77_live_probe_callback_shutdown) once (it returns a count). Then disable this plugin in CE, run pcall(ce77_live_probe_callback_shutdown) again, and preserve the raw pcall result. Re-enable and call ce77_live_probe_status().";
    }

    private static void RunSynchronizeProbe()
    {
        var observation = SynchronizeObservation.Started;
        try
        {
            var returnThread = MainThread.Invoke(static _ => SynchronizeProbeState.RecordSuccessfulInvocation(), 0);
            ...
            try
            {
                MainThread.Invoke(static _ => throw new InvalidOperationException("CE77-live-probe expected dispatch failure."), 0);
    ...
    private static class SynchronizeProbeState
    {
        ...
        {
            var workThread = Environment.CurrentManagedThreadId;
            var nestedThread = MainThread.Invoke(static _ => Environment.CurrentManagedThreadId, 0);
    ...
    private readonly record struct SynchronizeObservation(bool IsRunning, string Completion, int WorkerThreadId,
        int WorkThreadId, int NestedThreadId, string? ReturnRoundTrip, string? ExceptionResult, string? Failure)
    {
    ...
    private readonly record struct LuaThreadObservation(bool IsRunning, string Completion, nint GuiState, nint WorkerState,
        string ExpectedMarker, string? RegistryMarker, string RegistryType, bool RegistryMatches, string? Failure)
    {
        internal static LuaThreadObservation NotStarted => new(false, "not started", 0, 0, string.Empty, null, string.Empty, false, null);
    ...
    private readonly record struct ResetObservation(bool HasBefore, nint BeforeState, int BeforeEpoch, int BeforeReference,
        bool HasAfter, nint AfterState, int AfterEpoch, bool OldReferencePushed, string? OldReferenceValue)
    {
        ...
        internal string ToDisplayString()
        {
            if (!HasBefore) return "not started";
