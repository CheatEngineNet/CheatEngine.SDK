...
namespace CheatEngine.SDK.Lua.Interop.Tests.Fixture;
...
public sealed class NativeLuaProbeTests
{
	...
	{
		string relative = Path.Combine("cheatengine-sdk-no-such-directory", "lua53.dll");

		NativeLuaProbe probe = NativeLuaProbe.Run("  " + relative + "  ");
	...
	{
		NativeLuaProbe probe = NativeLuaProbe.Run("lua\0.dll");
	...
	{
		string missing = Path.Combine(Path.GetTempPath(), "cheatengine-sdk-no-such-directory", "lua53.dll");

		NativeLuaProbe probe = NativeLuaProbe.Run(missing);
