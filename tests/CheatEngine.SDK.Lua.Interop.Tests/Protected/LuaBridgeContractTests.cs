...
public sealed unsafe class LuaBridgeContractTests
{
	...
	public void Native_bridge_has_the_fixed_contract_and_no_Lua_import()
	{
		...
		try
		{
			for (int i = 0; i < s_fixedExports.Length; i++)
			{
		...
		List<string> imports = ReadImportedModules(path);
		...
		List<string> exports = ReadExportedNames(path);
	...
	private static ushort ReadMachine(string path)
	{
		...
		int peOffset = checked((int) ReadUInt32(image, 0x3c));
	...
	private static List<string> ReadExportedNames(string path)
	{
		...
		ushort optionalSize = ReadUInt16(image, peOffset + 20);
		int optionalOffset = peOffset + 24;
		Require(ReadUInt16(image, optionalOffset) == 0x20b, "The native bridge is not PE32+ (Windows x64).");
		Require(ReadUInt32(image, optionalOffset + 108) > 0, "The native bridge has no export-directory slot.");

		uint exportRva = ReadUInt32(image, optionalOffset + 112);
		int sectionOffset = optionalOffset + optionalSize;
		uint sizeOfHeaders = ReadUInt32(image, optionalOffset + 60);
		List<string> result = [];
		if (exportRva == 0)
		{
			return result;
		...
		int exportOffset = RvaToFileOffset(image, exportRva, sizeOfHeaders, sectionOffset, sectionCount);
		Require(exportOffset <= image.Length - 40, "The native bridge has a truncated export directory.");
		uint namesCount = ReadUInt32(image, exportOffset + 24);
		uint namesRva = ReadUInt32(image, exportOffset + 32);
		int namesOffset = RvaToFileOffset(image, namesRva, sizeOfHeaders, sectionOffset, sectionCount);
	...
	private static List<string> ReadImportedModules(string path)
	{
		...
		int importDirectoryOffset = optionalOffset + 112 + 8;
		uint importRva = ReadUInt32(image, importDirectoryOffset);
		int sectionOffset = optionalOffset + optionalSize;
		uint sizeOfHeaders = ReadUInt32(image, optionalOffset + 60);
		List<string> result = [];
		if (importRva == 0)
		...
		int descriptorOffset = RvaToFileOffset(image, importRva, sizeOfHeaders, sectionOffset, sectionCount);
		while (true)
		{
			...
			uint timeDateStamp = ReadUInt32(image, descriptorOffset + 4);
			uint forwarderChain = ReadUInt32(image, descriptorOffset + 8);
		...
		ushort sectionCount)
	{
		...
		for (int i = 0; i < sectionCount; i++)
		{
			...
			uint rawSize = ReadUInt32(image, offset + 16);
			uint rawOffset = ReadUInt32(image, offset + 20);
			uint sectionSize = Math.Max(virtualSize, rawSize);
			if (rva < virtualAddress || rva >= (ulong) virtualAddress + sectionSize)
			{
				continue;
			}
