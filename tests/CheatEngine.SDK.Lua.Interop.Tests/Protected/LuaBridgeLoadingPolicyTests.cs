...
public sealed class LuaBridgeLoadingPolicyTests
{
	...
	{
		Assembly assembly = typeof(LuaProtectedApi).Assembly;
		DefaultDllImportSearchPathsAttribute? policy =
			assembly.GetCustomAttribute<DefaultDllImportSearchPathsAttribute>();

		Assert.NotNull(policy);
		Assert.Equal(DllImportSearchPath.AssemblyDirectory, policy!.Paths);

		string bridgePath = Path.Combine(Path.GetDirectoryName(assembly.Location)!, "cheatengine-sdk-lua-bridge.dll");
		Assert.True(File.Exists(bridgePath), $"The native Lua bridge was not copied to '{bridgePath}'.");

		IntPtr module = NativeLibrary.Load(
