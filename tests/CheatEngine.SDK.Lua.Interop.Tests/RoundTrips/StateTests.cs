...
public sealed unsafe class StateTests
{
	...
	public void NewState_uses_the_managed_allocator_it_is_given()
	{
		...
		lua_State* L = lua_newstate(&CountingAllocator, &allocations);
		Assert.True(L is not null);
		try
		{
			void* userData;
	...
	public void Setallocf_replaces_the_allocator_that_getallocf_reports()
	{
		...
		lua_State* L = lua_newstate(&CountingAllocator, &first);
		Assert.True(L is not null);
		try
		{
			// Same function, other opaque pointer: the new allocator can free what the old one handed out.
			delegate* unmanaged[Cdecl]<void*, void*, UIntPtr, UIntPtr, void*> allocator = lua_getallocf(L, null);
			...
			long firstBefore = first;
			long secondBefore = second;
			lua_createtable(L, 16, 16);
	...
	public void Atpanic_returns_the_previous_function()
	{
		...
		delegate* unmanaged[Cdecl]<lua_State*, int> installedByAuxlib = lua_atpanic(state.L, mine);
		delegate* unmanaged[Cdecl]<lua_State*, int> previous = lua_atpanic(state.L, installedByAuxlib);
	...
	private static void* CountingAllocator(void* ud, void* ptr, nuint osize, nuint nsize)
	{
		...
		try
		{
			void* block = NativeMemory.Realloc(ptr, nsize);
			(*(long*) ud)++;
			return block;
		}
