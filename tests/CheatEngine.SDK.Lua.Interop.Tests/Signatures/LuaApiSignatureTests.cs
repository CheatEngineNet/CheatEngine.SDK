...
namespace CheatEngine.SDK.Lua.Interop.Tests.Signatures;
...
public sealed class LuaApiSignatureTests
{
	...
	public void Public_forwarders_repeat_the_slot_signature_exactly()
	{
		foreach (FieldInfo field in TableFields)
		{
			MemberInfo[] members = typeof(LuaApi).GetMember(field.Name, PublicStatic);
			MemberInfo forwarder = Assert.Single(members);
			...
			MethodInfo method = Assert.IsType<MethodInfo>(forwarder, false);
	...
	public void Public_forwarders_load_their_own_slot_and_pass_the_parameters_in_declared_order()
	{
		...
		foreach (FieldInfo field in TableFields)
		{
			MemberInfo forwarder = Assert.Single(typeof(LuaApi).GetMember(field.Name, PublicStatic));
			MethodInfo? method = forwarder is PropertyInfo property ? property.GetMethod : forwarder as MethodInfo;
			...
			int indirectCalls = 0;
			int otherCalls = 0;
			foreach ((OpCode code, int operand) in IlReader.Read(method))
			{
				if (code.OperandType == OperandType.InlineField)
				{
					FieldInfo? loaded = method.Module.ResolveField(operand);
					if (loaded?.DeclaringType == typeof(LuaApi.Table))
					{
						slots.Add(loaded.Name);
					}
				}
	...
	{
		Type[] parameters = SlotParameters("lua_pcallk");
	...
	{
		Type[] parameters = SlotParameters("lua_callk");
	...
	public void No_slot_uses_a_non_blittable_type()
	{
		foreach (FieldInfo field in TableFields)
		{
			Type signature = field.FieldType;
	...
	{
		FieldInfo? field = typeof(LuaApi.Table).GetField(name, AnyInstance);
