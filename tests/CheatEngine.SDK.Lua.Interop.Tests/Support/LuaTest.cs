...
internal static unsafe class LuaTest
{
	...
	{
		if (Load(L, source) != LUA_OK)
		{
			Assert.Fail("The chunk did not compile: " + ReadString(L, -1));
		}

		if (lua_pcallk(L, 0, nresults, 0, 0, null) != LUA_OK)
		{
			Assert.Fail("The chunk raised: " + ReadString(L, -1));
		}
	}
	...
	{
		nuint length;
		byte* bytes = lua_tolstring(L, idx, &length);
