...
public sealed class CallbackLifetimeConcurrencyTests
{
	...
	public async Task Release_waits_for_a_thunk_before_its_state_lookup_then_the_thunk_reports_released()
	{
		...
		{
			Counter counter = new();
			Assert.True(LuaCallback
				.TryCreate(main, PauseBeforeLookupFunction, counter, out LuaCallback<Counter>? callback)
				.IsOk);
	...
	public async Task A_thunk_that_already_acquired_state_can_finish_after_release_frees_its_handle()
	{
		...
		try
		{
			Counter counter = new();
			Assert.True(LuaCallback
				.TryCreate(main, PauseAfterLookupFunction, counter, out LuaCallback<Counter>? callback)
				.IsOk);
			Assert.NotNull(callback);
	...
	public async Task Detach_waits_for_an_admitted_callback_and_rejects_a_callback_that_starts_after_close()
	{
		...
		try
		{
			Counter counter = new();
			Assert.True(LuaCallback
				.TryCreate(main, PauseAfterLookupFunction, counter, out LuaCallback<Counter>? callback).IsOk);
			Assert.NotNull(callback);
			...
			LuaStatus late = main.TryExecute("local ok, err = pcall(count) return ok, err"u8, 2);
	...
	private static int PauseBeforeLookup(nint pointer)
	{
		...
		try
		{
			...
			bool found = LuaThunk.TryGetState(state, out Counter? _);
	...
	private static int PauseAfterLookup(nint pointer)
	{
		...
		{
			CallbackRace race = s_race ?? throw new InvalidOperationException("No callback race is active.");
			if (!LuaThunk.TryGetState(state, out Counter? counter))
			{
	...
	private sealed class CallbackRace(CancellationToken cancellationToken) : IDisposable
	{
		...
		public ManualResetEventSlim Continue
		{
			get;
		} = new(false);

		public ManualResetEventSlim LookupAttempted
		{
			get;
		} = new(false);

		public ManualResetEventSlim LookupReturned
		{
			get;
		} = new(false);
