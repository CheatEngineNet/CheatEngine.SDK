...
public sealed class LuaCallbackRegistryTests
{
	...
	{
		LuaCallback<object> tail = NewCallback();
		LuaCallback<object> middle = NewCallback();
		LuaCallback<object> head = NewCallback();
	...
	{
		LuaCallback<object> first = NewCallback();
		LuaCallback<object> second = NewCallback();
		LuaCallbackRegistry.Add(first);
		LuaCallbackRegistry.Add(second);
		LuaCallback<object> target = removeHead ? second : first;
	...
	{
		LuaCallback<object> callback = NewCallback();
		LuaCallback<object> other = NewCallback();
