...
public sealed class LuaHostSubscriptionTests
{
    ...
    public void Registration_failure_leaves_the_callback_inert_and_does_not_unregister()
    {
        ...
        var registered = LuaHostSubscription.TryRegister(L, () => calls++,
    ...
    public void Detach_unregisters_lifo_once_and_late_callbacks_are_inert()
    {
        ...
        Assert.True(LuaHostSubscription.TryRegister(L, static () => { },
            (registrationState, callback) => releaseState => releases.Add("hotkey-b"), out var second));
        Assert.True(LuaHostSubscription.TryRegister(L, static () => { },
            (registrationState, callback) =>
            {
    ...
    public void Reset_and_reenable_leave_the_old_subscription_callback_inert()
    {
        ...
        var oldIdentity = subscription.Identity;
    ...
    public void Reset_reopens_registration_admission_for_new_host_subscriptions()
    {
        ...
        var L = LuaTest.View(state);
        using RuntimeScope scope = new(state);
        var oldUnregisters = 0;
        var newUnregisters = 0;

        Assert.True(LuaHostSubscription.TryRegister(L, static () => { },
            (registrationState, callback) => releaseState => oldUnregisters++, out var oldSubscription));
        ...
        Assert.True(LuaHostSubscription.TryRegister(L, static () => { },
            (registrationState, callback) => releaseState => newUnregisters++, out var newSubscription));
    ...
    public async Task Disable_admission_waits_for_an_entered_callback_and_rejects_a_late_one()
    {
        ...
        using ManualResetEventSlim allowReturn = new(initialState: false);
        ...
        try
        {
            var enteredCall = Task.Factory.StartNew(hostCallback!, cancellationToken, TaskCreationOptions.LongRunning,
                TaskScheduler.Default);
            Assert.True(entered.Wait(TimeSpan.FromSeconds(5), cancellationToken),
                "The host callback did not enter its handler.");

            var close = Task.Factory.StartNew(LuaRuntime.CloseHostSubscriptionAdmissionAndDrain, cancellationToken,
    ...
    public async Task Disable_drain_does_not_hold_the_registry_gate_against_an_admitted_callback()
    {
        ...
        var L = LuaTest.View(state);
        using RuntimeScope scope = new(state);
        using var race = new DisableDrainRace(L, cancellationToken);
        ...
        try
        {
            var enteredCall = Task.Factory.StartNew(race.HostCallback!, cancellationToken, TaskCreationOptions.LongRunning,
                TaskScheduler.Default);
    ...
    private sealed class DisableDrainRace(LuaState state, CancellationToken cancellationToken) : IDisposable
    {
        ...
        public ManualResetEventSlim AllowCallbackActions { get; } = new(initialState: false);

        public ManualResetEventSlim CallbackDrainStarted { get; } = new(initialState: false);

        public ManualResetEventSlim CallbackActionsCompleted { get; } = new(initialState: false);

        public Action? HostCallback { get; private set; }

        public LuaHostSubscription? FirstSubscription { get; private set; }

        public LuaHostSubscription? SecondSubscription { get; private set; }

        public Exception? CallbackFailure { get; private set; }

        public int RejectedRegistrationUnregisters { get; private set; }

        public int SecondUnregisters { get; private set; }
        ...
        public bool Register()
        {
            if (!LuaHostSubscription.TryRegister(state, RunAdmittedCallback,
                    (registrationState, callback) =>
                    {
                        HostCallback = callback;
                        return static _ => { };
                    }, out var firstSubscription))
                return false;

            FirstSubscription = firstSubscription;
            return LuaHostSubscription.TryRegister(state, static () => { },
                (registrationState, callback) => releaseState => SecondUnregisters++, out var secondSubscription)
        ...
        private void RunAdmittedCallback()
        {
            ...
            try
            {
                ...
                var registered = LuaHostSubscription.TryRegister(state, static () => { },
                    (registrationState, callback) => releaseState => RejectedRegistrationUnregisters++,
                    out var rejectedSubscription);
