...
internal static unsafe class Thunks
{
	...
	private static int AddThunk(nint handle)
	{
		...
		try
		{
			if (!Int64Marshaller.TryRead(L, 1, out long a) || !Int64Marshaller.TryRead(L, 2, out long b))
			{
	...
	private static int GreetThunk(nint handle)
	{
		...
		try
		{
			if (!StringMarshaller.TryRead(L, 1, out string? name))
			{
				return LuaThunk.FailBadArgument(L, 1, "string"u8);
			}
	...
	private static int DeepStackFirstProtectedThunk(nint handle)
	{
		...
		try
		{
			for (int i = 0; i < DeepStackSlots; i++)
			{
				L.PushInteger(i);
			}

			LuaStatus status = L.TryGetGlobal("x"u8);
			if (!status.IsOk)
			{
				return LuaThunk.Fail(L, "TryGetGlobal failed at depth"u8);
			}

			// The pushed integers must be intact under the result.
			for (int i = 0; i < DeepStackSlots; i++)
			{
				if (!L.TryReadInteger(i + 1, out long value) || value != i)
				{
	...
	private static int SetUpvalueTableThunk(nint handle)
	{
		...
		try
		{
			...
			L.PushInteger(1);
			if (!L.TryRawSet(table))
			{
				return LuaThunk.Fail(L, "raw set failed"u8);
			}
