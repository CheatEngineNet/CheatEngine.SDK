...
internal static partial class MemoryBindings
{
	...
	private static partial bool TryReadInt32Raw(nuint address, bool signed, out int value)
	{
		...
		bool ok = Int32Marshaller.TryRead(L, -1, out value); // tointegerx + range check; nil => false, no exception
		L.SetTop(top);
		return ok;
	}
	...
	private static partial int ReadInt32Raw(nuint address, bool signed)
	{
		...
		BooleanMarshaller.Push(L, signed);
		LuaStatus status = L.TryCall(2, 1);
		if (!status.IsOk)
		{
			LuaCallSupport.Throw(L, top, status); // exit 2: the call raised
		}

		if (!Int32Marshaller.TryRead(L, -1, out int value))
		{
