...
public sealed class ReadIntegerBindingTests
{
	...
	public void Reads_a_value_the_stand_in_knows()
	{
		...
		Assert.True(MemoryBindings.TryReadInt32(0x1000, out int value));

		Assert.Equal(42, value);
		Assert.True(MemoryBindings.TryReadInt32(0x1004, out int negative));
	...
	public void Nil_result_is_false_without_an_exception()
	{
		...
		Assert.False(MemoryBindings.TryReadInt32(0x2000, out int value));
	...
	public void A_raising_global_is_false_and_leaves_the_stack_balanced()
	{
		...
		Assert.False(MemoryBindings.TryReadInt32(0xDEAD, out int value));

		Assert.Equal(0, value);
		Assert.Equal(0, L.Top);
		Assert.True(MemoryBindings.TryReadInt32(0x1000, out int after));
	...
	public void The_cached_reference_is_re_resolved_after_the_host_re_attaches()
	{
		...
		LuaState L1 = LuaTest.View(first);
		LuaState L2 = LuaTest.View(second);
	...
	public void The_throwing_form_has_one_exception_per_exit_and_names_the_type_of_an_unexpected_result()
	{
		...
		LuaException unresolved = Assert.Throws<LuaException>(() => MemoryBindings.ReadInt32(0x1000));
		...
		LuaException raised = Assert.Throws<LuaException>(() => MemoryBindings.ReadInt32(0xDEAD));
		...
		LuaException nil = Assert.Throws<LuaException>(() => MemoryBindings.ReadInt32(0x2000));
		Assert.Equal("The Lua global 'readInteger' returned a nil value, not an integer.", nil.Message);
		LuaException fraction = Assert.Throws<LuaException>(() => MemoryBindings.ReadInt32(0x100C));
	...
	public void The_generated_shape_allocates_nothing()
	{
		...
		{
			if (!MemoryBindings.TryReadInt32(0x1000, out int value) || value != 42)
			{
				throw new InvalidOperationException("wrong value");
			}

			if (MemoryBindings.TryReadInt32(0x2000, out _))
			{
