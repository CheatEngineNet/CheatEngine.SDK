...
public sealed class LuaRuntimeDetachedTests
{
	...
	public void Detached_runtime_reports_itself_as_such()
	{
		...
		Assert.False(LuaRuntime.TryAcquireState(out LuaState state));
	...
	public void AcquireState_throws_while_detached()
	{
		...
		InvalidOperationException exception = Assert.Throws<InvalidOperationException>(() => LuaRuntime.AcquireState());
	...
	{
		using LuaStateResetTransition reset = LuaRuntime.BeginStateReset();
	}
	...
	public void Attach_rejects_a_binding_without_a_state_provider()
	{
		...
		ArgumentException exception = Assert.Throws<ArgumentException>(() => LuaRuntime.Attach(in invalid));
