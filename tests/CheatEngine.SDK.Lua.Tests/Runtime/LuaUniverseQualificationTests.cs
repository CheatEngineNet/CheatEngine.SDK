...
public sealed class LuaUniverseQualificationTests
{
    ...
    public void First_worker_acquisition_rejects_a_missing_state_then_uses_a_distinct_coroutine_in_the_same_universe()
    {
        ...
        var sharedReference = main.CreateRef();
        using var worker = RootedCoroutine.Create(main);
        var identity = LuaRuntime.CurrentStateIdentity;
        var observation = ObserveFirstWorker(worker, sharedReference);
    ...
    [Fact]
    public void State_reset_generation_invalidates_shared_registry_references_and_callbacks_before_a_fresh_worker_state()
    {
        LuaTest.RequireNativeLua();
        using NativeLuaState state = new();
        var main = LuaTest.View(state);
        ...
        var replacementMain = LuaTest.View(replacementState);
        using var replacementWorker = RootedCoroutine.Create(replacementMain);
        main.PushString("before reset"u8);
        var reference = main.CreateRef();
        Assert.True(LuaCallback.TryCreate(main, ReturnOneFunction, new object(), out var callback).IsOk);
        ...
        var observation = ObservePostResetWorker(replacementWorker, reference, callback);
        ...
        FirstWorkerObservation observation)
    {
        try
        {
            ...
            observation.FirstAcquisitionWasRejected = !LuaRuntime.TryAcquireOperation(out var unavailable);
            unavailable.Dispose();
            HostDouble.SetStateForCurrentThread(worker.State.Handle);
            using var operation = LuaRuntime.AcquireOperation();
            var workerLua = operation.State;
            ...
            Assert.True(workerLua.TryReadInteger(-1, out var globalValue));
            observation.GlobalValue = globalValue;
            workerLua.Pop(1);
            Assert.True(workerLua.TryPushRef(sharedReference));
            Assert.True(workerLua.TryReadString(-1, out var referenceValue));
    ...
    private sealed class FirstWorkerObservation
    {
        ...
        public bool FirstAcquisitionWasRejected { get; set; }

        public long GlobalValue { get; set; }

        public string? ReferenceValue { get; set; }

        public LuaStateIdentity WorkerIdentity { get; set; }

        public nint WorkerState { get; set; }
    }

    private sealed class PostResetWorkerObservation
    {
        public bool CallbackPushed { get; set; } = true;

        public string CallbackError { get; set; } = string.Empty;

        public LuaStatus CallbackStatus { get; set; }

        public Exception? Failure { get; set; }

        public bool ReferencePushed { get; set; } = true;
    ...
    private sealed unsafe class RootedCoroutine : IDisposable
    {
        ...
        {
            var status = main.TryExecute("return coroutine.create(function() end)"u8, 1);
            Assert.True(status.IsOk);
            var pointer = lua_tothread(main.Pointer, -1);
