...
public sealed class LuaStateStringTests
{
	...
	public void Utf8_bytes_with_embedded_nul_round_trip_exactly()
	{
		...
		ReadOnlySpan<byte> bytes = "ab\0cd\0"u8;

		Utf8Marshaller.Push(L, bytes);

		Assert.True(Utf8Marshaller.TryRead(L, -1, out ReadOnlySpan<byte> read));
		Assert.True(read.SequenceEqual(bytes));
		Assert.Equal((nuint) 6, L.RawLength(-1));
		Assert.True(L.TryReadString(-1, out string? text));
	...
	public void Non_ascii_text_round_trips_through_utf8()
	{
		...
		Assert.True(L.TryReadUtf8(-1, out ReadOnlySpan<byte> utf8));
		Assert.True(utf8.SequenceEqual(Encoding.UTF8.GetBytes(Text)));
		Assert.True(StringMarshaller.TryRead(L, -1, out string? read));
	...
	public void Lua_sees_the_bytes_the_sdk_pushed()
	{
		...
		L.PushString("\u00E9x");
		LuaStatus status = L.TryCall(1, 3);

		Assert.True(status.IsOk);
		Assert.True(L.TryReadInteger(-3, out long length));
		Assert.Equal(3, length);
		Assert.True(L.TryReadInteger(-2, out long first));
		Assert.Equal(0xC3, first);
		Assert.True(L.TryReadInteger(-1, out long last));
		Assert.Equal('x', last);
	}
	...
	public void A_string_longer_than_the_stack_buffer_is_pushed_through_the_pool()
	{
		...
		using NativeLuaState state = new(false);
		LuaState L = LuaTest.View(state);
		string text = string.Concat(Enumerable.Repeat("\u00E9\u20AC-", 1_000));

		L.PushString(text);

		Assert.Equal((nuint) Encoding.UTF8.GetByteCount(text), L.RawLength(-1));
		Assert.True(L.TryReadString(-1, out string? read));
		Assert.Equal(text, read);
	}
	...
	public void Empty_strings_round_trip()
	{
		...
		Assert.True(L.TryReadUtf8(1, out ReadOnlySpan<byte> a));
		Assert.True(a.IsEmpty);
		Assert.True(L.TryReadString(2, out string? b));
	...
	public void String_reads_are_strict_a_number_is_not_converted_in_place()
	{
		...
		Assert.False(L.TryReadUtf8(-1, out ReadOnlySpan<byte> utf8));
		Assert.True(utf8.IsEmpty);
		Assert.False(L.TryReadString(-1, out string? text));
		Assert.Null(text);
