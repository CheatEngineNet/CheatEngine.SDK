...
internal static class Program
{
    ...
    private static LibraryInspection ReadAndValidateProbe(string libraryPath)
    {
        ...
        var fileLock = new FileStream(fullPath, FileMode.Open, FileAccess.Read, FileShare.Read);
        byte[] bytes;
        try
        {
    ...
    private static void EnsureExpectedExports(IReadOnlyList<string> exportNames)
    {
        foreach (string requiredName in NativeAotLibraryProbeExportNames.Required)
        {
            ...
            foreach (string exportName in exportNames)
            {
                if (!string.Equals(exportName, requiredName, StringComparison.Ordinal)) continue;
            ...
            if (!found)
                throw new InvalidOperationException($"The file does not expose required fixture export '{requiredName}'.");
        }
    }
