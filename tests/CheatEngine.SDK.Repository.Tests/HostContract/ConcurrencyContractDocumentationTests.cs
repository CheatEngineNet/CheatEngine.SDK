...
public sealed class ConcurrencyContractDocumentationTests
{
	...
	private const string SelfPath = "tests/CheatEngine.SDK.Repository.Tests/HostContract/ConcurrencyContractDocumentationTests.cs";
	...
	public void Sources_and_readmes_never_claim_one_runtime_or_load_context_per_plugin()
	{
		...
		Assert.True(offenders.Count == 0,
			"Unqualified plugin-cardinality claim(s):" + Environment.NewLine + string.Join(Environment.NewLine, offenders));
	}

	[Fact]
	public void Hosting_readme_publishes_the_ADR_07_concurrency_contract()
	{
		string readme = File.ReadAllText(Path.Combine(RepositoryRoot.Path, "libs", "CheatEngine.SDK.Hosting", "README.md"));
