...
public sealed class NoOutputTests(RoslynFixture roslyn) : IClassFixture<RoslynFixture>
{
	...
	{
		GeneratorRun run = roslyn.Run("notes.txt", SpecSources.SingleTry);
	...
	{
		GeneratorRun run = roslyn.Run("empty.cheatengine-sdk-api.txt", string.Empty);
	...
	{
		GeneratorRun run = roslyn.Run("shell.cheatengine-sdk-api.txt", "namespace: Demo\ntype: Shell\n");
	...
	public void A_spec_file_whose_only_entry_is_invalid_produces_no_output_and_reports_a_specification_error()
	{
		...
		GeneratorRun run = roslyn.Run("bad.cheatengine-sdk-api.txt", Text);
	...
	{
		GeneratorRun run = GeneratorRun.Execute(RoslynFixture.CreateDriver(), roslyn.CreateCompilation());
