...
public sealed class KnownLimitationTests(RoslynFixture roslyn) : IClassFixture<RoslynFixture>
{
	...
	public void Generator_sdk_referenced_only_through_an_extern_alias_fails_in_generated_code_with_CS0400()
	{
		...
		ImmutableArray<MetadataReference> references = roslyn.Environment.FrameworkReferences
	...
	public void Generator_consumer_below_csharp_11_fails_in_generated_code_with_language_version_errors()
	{
		...
		CSharpParseOptions csharp10 = RoslynEnvironment.ParseOptions.WithLanguageVersion(LanguageVersion.CSharp10);
		CSharpCompilation compilation =
			RoslynFixture.CreateCompilation(roslyn.Environment, csharp10, PluginSources.Nominal);

		GeneratorRun run = RoslynFixture.Run(compilation, parseOptions: csharp10);
