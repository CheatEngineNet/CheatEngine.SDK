...
public sealed class LanguageVersionTests(RoslynFixture roslyn) : IClassFixture<RoslynFixture>
{
	...
	{
		CSharpParseOptions parseOptions = RoslynEnvironment.ParseOptions.WithLanguageVersion(version);
		CSharpCompilation compilation =
			RoslynFixture.CreateCompilation(roslyn.Environment, parseOptions, PluginSources.Nominal);

		// The driver parses the generated tree with the consumer's language version, like the compiler does.
		GeneratorRun run = RoslynFixture.Run(compilation, parseOptions: parseOptions);
