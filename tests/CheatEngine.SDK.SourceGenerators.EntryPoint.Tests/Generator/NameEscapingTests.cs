...
public sealed class NameEscapingTests(RoslynFixture roslyn) : IClassFixture<RoslynFixture>
{
    ...
    {
        var (name, expectedFragment) = Cases[caseKey];

        // Roslyn's own literal formatter writes the attribute argument: independent of the escaper under test.
        var run = roslyn.Run(PluginSources.WithNameExpression(SymbolDisplay.FormatLiteral(name, true)));
    ...
    {
        var verbatim = roslyn.Run(PluginSources.WithNameExpression("@\"C:\\dir \"\"quoted\"\" \""));
        var raw = roslyn.Run(PluginSources.WithNameExpression("\"\"\"C:\\dir \"quoted\" \"\"\""));
