...
public sealed class BootstrapModelTests
{
	...
	{
		BootstrapModel? model = BootstrapModel.Select(new EquatableArray<PluginModel>([ValidA]), On);
	...
	{
		BootstrapModel? model = BootstrapModel.Select(new EquatableArray<PluginModel>([Invalid, ValidB, Invalid]), On);
