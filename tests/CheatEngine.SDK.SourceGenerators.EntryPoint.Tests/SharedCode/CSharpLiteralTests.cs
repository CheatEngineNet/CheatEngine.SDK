...
public sealed class CSharpLiteralTests
{
	...
	{
		foreach (string value in RoundTripValues)
		{
			string literal = CSharpLiteral.ToStringLiteral(value);

			Assert.All(literal, static c => Assert.InRange(c, ' ', '~'));
			LiteralExpressionSyntax expression =
				Assert.IsType<LiteralExpressionSyntax>(SyntaxFactory.ParseExpression(literal));
	...
	public void ToUtf8Literal_output_is_lexed_back_to_the_utf8_encoding_of_the_value()
	{
		foreach (string value in RoundTripValues)
		{
			string literal = CSharpLiteral.ToUtf8Literal(value);

			Assert.All(literal, static c => Assert.InRange(c, ' ', '~'));
			LiteralExpressionSyntax expression = Assert.IsType<LiteralExpressionSyntax>(
