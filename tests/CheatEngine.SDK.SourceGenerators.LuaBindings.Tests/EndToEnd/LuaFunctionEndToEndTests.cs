...
public sealed class LuaFunctionEndToEndTests(RoslynFixture roslyn) : IClassFixture<RoslynFixture>
{
	...
	public void Void_function_returns_no_value_and_runs_the_target()
	{
		...
		FieldInfo pings = assembly.Assembly.GetType(SuiteType, true)!.GetField("Pings")!;
	...
	public void Unregister_removes_the_globals_and_registration_can_run_again()
	{
		...
		LuaStatus unregistered = Invoke(assembly, "UnregisterLuaFunctions", L);
	...
	public void Lease_registration_preserves_a_later_global_replacement_and_is_idempotent()
	{
		...
		using RuntimeScope scope = new(state);
		GeneratedAssembly assembly = LoadSuite(roslyn);
		LuaRegistrationResult registration = (LuaRegistrationResult) assembly.Method(SuiteType,
			"TryRegisterLuaFunctions",
			[typeof(LuaState), typeof(LuaRegistrationCollisionPolicy)]).Invoke(null,
			[L, LuaRegistrationCollisionPolicy.RejectExisting])!;
		LuaRegistrationLease lease = Assert.IsType<LuaRegistrationLease>(registration.Lease);
	...
	public void Registration_failure_follows_the_status_protocol()
	{
		...
		LuaStatus status = Invoke(assembly, "RegisterLuaFunctions", L);
	...
	public void Retained_generated_closure_is_rejected_after_disable_and_reenable()
	{
		...
		using (new RuntimeScope(state))
		{
			...
			string message = LuaTest.RunForError(L, "return pcall(function() return savedAdd(1, 2) end)"u8);
	...
	public void Nil_is_rejected_for_a_nullable_string_argument_like_any_other_string_argument()
	{
		...
		LuaState L = LuaTest.View(state);
		using RuntimeScope scope = new(state);
		GeneratedAssembly assembly = GeneratedAssembly.Load(roslyn.Run(Source));
		LuaStatus registered =
			(LuaStatus) assembly.Method("Demo.NullableArgs", "RegisterLuaFunctions").Invoke(null, [L])!;
	...
	public void Calling_a_thunk_from_lua_allocates_nothing_on_the_managed_side()
	{
		...
		AllocationGate.AssertZero(() =>
		{
			...
			LuaStatus status = L.TryGetGlobal("loop"u8);
			if (!status.IsOk || !L.TryCall(0, 1).IsOk || !L.TryReadInteger(-1, out long sum) || sum != 5050)
			{
