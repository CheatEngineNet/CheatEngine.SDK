...
public sealed class LuaGlobalEndToEndTests(RoslynFixture roslyn) : IClassFixture<RoslynFixture>
{
	...
	public void Try_form_reads_values_and_reports_nil_wrong_kind_and_raising_global_as_false()
	{
		...
		Assert.True(tryRead(0x1004, out int negative));
		Assert.Equal(-7, negative);

		Assert.False(tryRead(0x2000, out int text)); // a string, not an integer
		Assert.Equal(0, text);
		Assert.False(tryRead(0x3000, out int missing)); // nil
		Assert.Equal(0, missing);
		Assert.False(tryRead(0x1008, out _)); // does not fit 32 bits
		Assert.False(tryRead(0x100C, out _)); // 2.5
		Assert.False(tryRead(0xDEAD, out int raised)); // error('access violation')
	...
	public void Outcome_form_preserves_the_factual_lua_cause_without_reading_error_text()
	{
		...
		using NativeLuaState state = new();
		LuaState L = LuaTest.View(state);
		TryReadInt32DetailedDelegate detailed =
			LoadSuite(roslyn).Delegate<TryReadInt32DetailedDelegate>(BindingsType, "TryReadInt32Detailed");
		...
		{
			Assert.Equal(expected[i], actual[i].Kind);
		}
	...
	public void Outcome_void_form_preserves_resolution_call_failure_and_success()
	{
		...
		using NativeLuaState state = new();
		LuaState L = LuaTest.View(state);
		BeepDetailedDelegate detailed = LoadSuite(roslyn).Delegate<BeepDetailedDelegate>(BindingsType, "BeepDetailed");
	...
	public void Throwing_form_returns_the_value_and_throws_once_per_exit_with_the_lua_message()
	{
		...
		LuaState L = LuaTest.View(state);
		using RuntimeScope scope = new(state);
		ReadInt32Delegate read = LoadSuite(roslyn).Delegate<ReadInt32Delegate>(BindingsType, "ReadInt32");

		// Exit 1: the global is not defined yet.
		LuaException unresolved = Assert.Throws<LuaException>(() => read(0x1000));
		...
		LuaException raised = Assert.Throws<LuaException>(() => read(0xDEAD));
	...
	public void String_results_come_back_copied_out_as_string_and_as_return_value()
	{
		...
		TryReadStringCopyDelegate copyOut = assembly.Delegate<TryReadStringCopyDelegate>(BindingsType, "TryReadString");
		TryReadStringDelegate asString = assembly.Delegate<TryReadStringDelegate>(BindingsType, "TryReadString");
		ReadStringDelegate throwing = assembly.Delegate<ReadStringDelegate>(BindingsType, "ReadString");

		Span<byte> buffer = stackalloc byte[32];
		Assert.True(copyOut(0x2000, 5, buffer, out int written));
		Assert.Equal("hello", Encoding.UTF8.GetString(buffer[..written]));
		Assert.False(copyOut(0x2000, 100, buffer[..3], out int tooSmall)); // 12 bytes do not fit 3
		...
		Assert.True(asString(0x2000, 12, out string? text));
		Assert.Equal("hello, world", text);
		Assert.False(asString(0x3000, 12, out string? missing));
		Assert.Null(missing);
	...
	public void Void_and_boolean_throwing_forms_run_the_global()
	{
		...
		LuaTest.Run(L, StandIns);
		GeneratedAssembly assembly = LoadSuite(roslyn);
		BeepDelegate beep = assembly.Delegate<BeepDelegate>(BindingsType, "Beep");
		IsKeyPressedDelegate isKeyPressed = assembly.Delegate<IsKeyPressedDelegate>(BindingsType, "IsKeyPressed");
	...
	public void Several_results_are_read_and_all_defaulted_together_on_failure()
	{
		...
		LuaTest.Run(L, StandIns);
		GeneratedAssembly assembly = LoadSuite(roslyn);
		TryDivideDelegate divide = assembly.Delegate<TryDivideDelegate>(BindingsType, "TryDivide");
		TryDescribeDelegate describe = assembly.Delegate<TryDescribeDelegate>(BindingsType, "TryDescribe");

		Assert.True(divide(17, 5, out long quotient, out long remainder));
		...
		Assert.False(divide(17, 0, out long noQuotient, out long noRemainder));
		Assert.Equal(0, noQuotient);
		Assert.Equal(0, noRemainder);

		Assert.True(describe(1.5, true, out string? text, out double doubled));
	...
	public void Leading_state_parameter_is_used_instead_of_the_runtime()
	{
		...
		LuaTest.Run(L, StandIns);
		GeneratedAssembly assembly = LoadSuite(roslyn);
		AddOnDelegate add = assembly.Delegate<AddOnDelegate>(BindingsType, "AddOn");
		TryAddOnDelegate tryAdd = assembly.Delegate<TryAddOnDelegate>(BindingsType, "TryAddOn");

		// The runtime must be attached for the epoch-checked cache, but the state comes from the argument.
		using RuntimeScope scope = new(state);
		Assert.Equal(5, add(L, 2, 3));
		Assert.True(tryAdd(L, 40, 2, out long sum));
		Assert.Equal(42, sum);
		Assert.Equal(0, L.Top);
	}
	...
	public void Utf8_and_nullable_string_arguments_are_pushed()
	{
		...
		LuaTest.Run(L, StandIns);
		GeneratedAssembly assembly = LoadSuite(roslyn);
		UpperDelegate upper = assembly.Delegate<UpperDelegate>(BindingsType, "Upper");
		UpperOrNullDelegate upperOrNull = assembly.Delegate<UpperOrNullDelegate>(BindingsType, "UpperOrNull");
	...
	public void Warm_try_form_allocates_nothing()
	{
		...
		{
			if (!tryRead(0x1000, out int value) || value != 42)
			{
				throw new InvalidOperationException("wrong value");
			}

			if (tryRead(0x3000, out _))
			{
	...
	public void Warm_copy_out_string_form_allocates_nothing()
	{
		...
		using RuntimeScope scope = new(state);
		LuaTest.Run(L, StandIns);
		TryReadStringCopyDelegate copyOut =
			LoadSuite(roslyn).Delegate<TryReadStringCopyDelegate>(BindingsType, "TryReadString");
		byte[] buffer = new byte[32];

		AllocationGate.AssertZero(() =>
		{
			if (!copyOut(0x2000, 5, buffer, out int written) || written != 5)
			{
				throw new InvalidOperationException("wrong value");
			}
		});
