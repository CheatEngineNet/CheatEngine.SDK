...
public sealed class LuaObjectOutputTests(RoslynFixture roslyn) : IClassFixture<RoslynFixture>
{
	...
	public void Borrowed_handle_generates_identity_marshalling_and_protected_members()
	{
		...
		string handle = run.GeneratedText("Demo.Scan.LuaClass.g.cs");
		...
		string members = run.GeneratedText("Demo.Scan.LuaObjectMembers.g.cs");
		...
		Assert.Contains("this.Handle.TryGetProperty<global::CheatEngine.SDK.Lua.Marshalling.Int32Marshaller, int>",
			members,
			StringComparison.Ordinal);
		Assert.Contains("this.Handle.TrySetProperty<global::CheatEngine.SDK.Lua.Marshalling.Int32Marshaller, int>",
			members,
			StringComparison.Ordinal);
	...
	public void Object_method_parameter_named_handle_does_not_shadow_the_generated_property()
	{
		...
		run.AssertCompilesClean();
		Assert.Contains("this.Handle.TryPushMethodLeavingObject",
			run.GeneratedText("Demo.ObjectHandle.LuaObjectMembers.g.cs"),
			StringComparison.Ordinal);
	}
	...
	{
		string newModifier = memberName is "Equals" or "GetHashCode" ? "new " : string.Empty;
		string source = $$"""
		                  using CheatEngine.SDK.Annotations.Lua;
	...
	public void Object_wide_arguments_and_results_preflight_the_stack_before_any_push()
	{
		...
		GeneratorRun run = roslyn.Run(source);
		string members = run.GeneratedText("Demo.Wide.LuaObjectMembers.g.cs");
		int stackCheck = members.IndexOf("if (!__ceState.TryEnsureStack(17))", StringComparison.Ordinal);
		int receiverPush = members.IndexOf("this.Handle.TryPushMethodLeavingObject(__ceState, \"sum15\"u8)",
			StringComparison.Ordinal);
	...
	public void Object_try_method_defaults_every_result_when_a_marshaller_push_throws()
	{
		...
		GeneratorRun run = roslyn.Run(source);
		string members = run.GeneratedText("Demo.Probe.LuaObjectMembers.g.cs");
		CompilationUnitSyntax root = RoslynFixture.Parse(members, "Demo.Probe.LuaObjectMembers.g.cs")
			.GetCompilationUnitRoot(TestContext.Current.CancellationToken);
		MethodDeclarationSyntax method = FindGeneratedMethod(root, "TryDescribe");
		Assert.NotNull(method.Body);
		TryStatementSyntax luaCall = FindTryStatement(method.Body!);
		CatchClauseSyntax exceptionCatch = Assert.Single(luaCall.Catches);
	...
	public void Partial_property_modifiers_and_accessor_visibility_are_preserved()
	{
		...
		GeneratorRun run = roslyn.Run(source);
		string members = run.GeneratedText("Demo.Properties.LuaObjectMembers.g.cs");
	...
	{
		GeneratorRun globalsOnly = RoslynFixture.Run(roslyn.CreateCompilation(RoslynEnvironment.SafeCompilationOptions,
		...
		GeneratorRun mixed = RoslynFixture.Run(roslyn.CreateCompilation(RoslynEnvironment.SafeCompilationOptions,
