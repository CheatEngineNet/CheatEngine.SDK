...
internal static class LuaTest
{
	...
	{
		LuaStatus status = L.TryExecute(source, resultCount, "=test"u8);
		if (!status.IsOk)
		{
			LuaError error = LuaError.FromStack(L, status);
	...
	public static long RunForInteger(LuaState L, ReadOnlySpan<byte> source)
	{
		using LuaFrame frame = new(L);
		Run(L, source, 1);
		Assert.True(L.TryReadInteger(-1, out long value), "The chunk returned a " + L.TypeOf(-1) + ", not an integer.");
		return value;
	}
	...
	{
		Assert.True(L.TryReadString(index, out string? value),
