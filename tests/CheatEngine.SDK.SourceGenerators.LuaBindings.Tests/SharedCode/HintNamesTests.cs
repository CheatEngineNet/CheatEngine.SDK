...
public sealed class HintNamesTests
{
	...
	{
		string hint = HintNames.ForType("Demo.Caf\u00E9", ".g.cs");
	...
	{
		string a = HintNames.ForType("Demo.Caf\u00E9", ".g.cs");
		string b = HintNames.ForType("Demo.Caf\u00E8", ".g.cs");
		string c = HintNames.ForType("Demo.Caf/", ".g.cs");
	...
	public void AllocateUnique_prepopulated_readable_name_uses_the_deterministic_hash_candidate()
	{
		...
		string readable = HintNames.ForType(TypeName, Suffix);
		HashSet<string> used = HintNames.CreateUsedNames();
		Assert.True(used.Add(readable));

		string hint = HintNames.AllocateUnique(TypeName, Suffix, used);
	...
	public void AllocateUnique_third_collision_uses_the_first_available_ordinal_suffix()
	{
		...
		const string Suffix = ".g.cs";
		string readable = HintNames.ForType(TypeName, Suffix);
		string hashed = HintNames.Disambiguated(TypeName, Suffix);
		string second = WithOrdinal(hashed, Suffix, 2);
		HashSet<string> used = HintNames.CreateUsedNames();
		Assert.True(used.Add(readable));
