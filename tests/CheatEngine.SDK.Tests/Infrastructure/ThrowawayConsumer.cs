...
internal sealed class ThrowawayConsumer
{
		...
		bool includeValueScanConsumer = false)
	{
		...
		string assemblyPath = Path.Combine(directory, "bin", "Release", "net10.0", $"{name}.dll");
		...
		string carrierPackageVersion, string localFeedDirectory)
	{
		...
		string directory = Path.Combine(parentDirectory, consumerName);
		System.IO.Directory.CreateDirectory(directory);

		string projectPath = Path.Combine(directory, $"{consumerName}.csproj");
		File.WriteAllText(projectPath, $"""
		                                <Project Sdk="Microsoft.NET.Sdk">
		                                  <PropertyGroup>
		                                    <TargetFramework>net10.0</TargetFramework>
		...
		string assemblyPath = Path.Combine(directory, "bin", "Release", "net10.0", $"{consumerName}.dll");
		return new ThrowawayConsumer(directory, projectPath, assemblyPath);
	}
	...
	private static void WriteNuGetConfig(string directory, string localFeedDirectory, params string[] localPackageIds)
	{
		...
		foreach (string packageId in localPackageIds)
		{
	...
	{
		string switches = string.Join(' ', propertyNames.Select(static p => $"-getProperty:{p}"));
		...
		int jsonStart = result.StandardOutput.AsSpan().IndexOf('{');
		if (jsonStart < 0)
		{
			throw new InvalidOperationException(
		...
		{
			values[name] = properties.TryGetProperty(name, out JsonElement value)
